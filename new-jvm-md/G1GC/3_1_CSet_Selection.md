...
CSet 选择的本质是**在停顿时间预算内最大化回收效益的贪心算法**：Young Region 全部进 CSet（必须回收），Old Region 按"垃圾密度"（`reclaimable_bytes`）降序排列，贪心地选取直到预测停顿时间达到 `MaxGCPauseMillis`。

### 0.2 为什么需要？

G1 的核心承诺是"可预测的停顿时间"。如果把所有 Old Region 都加入 CSet，停顿时间会超标；如果只回收 Young Region，Old 区会持续增长。CSet 选择在"回收足够多的 Old Region"和"停顿时间不超标"之间找到平衡。

### 0.3 怎么解决？

**两阶段 CSet 构建**：(1) `finalize_initial_collection_set()`：将所有 Eden + Survivor Region 加入 CSet，计算 Young 部分的预测停顿时间；(2) `finalize_old_part_of_collection_set()`（Mixed GC 时）：从 `CollectionSetChooser` 按垃圾密度降序取 Old Region，直到预测停顿时间达到目标。

### 0.4 为什么这样设计？

- **为什么 Young Region 必须全部进 CSet？** Young Region 不回收会持续增长，Eden 满了就触发 GC；Young GC 的本质就是清空 Eden，必须全部回收
- **为什么 Old Region 按垃圾密度排序？** 垃圾密度高的 Region 回收效率高（复制的存活对象少，释放的空间多）；贪心选择垃圾密度最高的 Region，在停顿时间预算内最大化回收空间
