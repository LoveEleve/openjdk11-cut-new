...
DirtyCardQueue 的本质是**写屏障与 RSet 更新之间的异步缓冲队列**：应用线程写屏障只做最轻量的操作（将脏卡地址入线程本地队列），后台 Concurrent Refinement 线程从全局 `DirtyCardQueueSet` 取出脏卡，扫描卡内引用，更新目标 Region 的 RSet。

### 0.2 为什么需要？

RSet 更新涉及复杂的数据结构操作（Sparse/Fine/Coarse 三级结构的查找+插入），代价高（微秒级）。如果在写屏障中同步更新 RSet，会严重拖慢应用线程（写屏障在热路径上，必须纳秒级）。DirtyCardQueue 将 RSet 更新推迟到后台线程，应用线程只需入队（几纳秒）。

### 0.3 怎么解决？

**线程本地缓冲 + 全局队列集合**：每个 Java 线程有一个 `DirtyCardQueue`（线程本地缓冲，默认 256 个槽位）；缓冲满时 flush 到全局 `DirtyCardQueueSet`；Concurrent Refinement 线程从 `DirtyCardQueueSet` 取缓冲区处理；GC 开始时 `update_rem_set()` 处理所有残留缓冲区。

### 0.4 为什么这样设计？

- **为什么线程本地缓冲默认 256 个槽位？** 太小：频繁 flush 到全局队列，锁竞争多；太大：GC 开始时残留脏卡多，增加 STW 时间；256 是经验最优值
- **为什么 flush 时整个缓冲区一起提交而不是逐个提交？** 批量提交减少全局队列的锁竞争次数；一次锁操作提交 256 个脏卡，比 256 次锁操作各提交 1 个高效得多
- **为什么需要全局 `DirtyCardQueueSet` 而不是每个线程直接被 Refinement 线程处理？** 全局队列解耦了生产者（应用线程）和消费者（Refinement 线程）；Refinement 线程数量少于应用线程数量，全局队列作为缓冲
