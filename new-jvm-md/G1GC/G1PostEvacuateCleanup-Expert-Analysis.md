...
G1PostEvacuateCleanup 的本质是**Evacuation 完成后的 STW 收尾工作**：释放 CSet 中的 Region（变为 Free）、更新 RSet（处理 Evacuation 期间产生的脏卡）、处理 Evacuation Failure（修复原地保留的对象）、更新并发标记状态（如果 Initial Mark 搭便车）。

### 0.2 为什么需要？

Evacuation 完成后，CSet 中的 Region 已经被清空（存活对象已复制走），但还有一些收尾工作：(1) 将空 Region 加入 Free List（供下次分配）；(2) 处理 Evacuation 期间产生的脏卡（Evacuation 本身也会产生跨 Region 引用）；(3) 如果有 Evacuation Failure，修复原地保留的对象的引用。

### 0.3 怎么解决？

**并行任务 + 串行收尾**：`G1PostEvacuateCollectionSetCleanupTask` 并行执行多个清理任务（每个 GC Worker 负责一部分 Region）；串行收尾：更新 `G1CollectorState`、触发并发标记（如果需要）、更新统计信息。

### 0.4 为什么这样设计？

- **为什么 Post-Evacuate Cleanup 是 STW 而不是并发？** 释放 CSet Region 需要更新 `HeapRegionManager` 的 Free List（需要同步）；修复 Evacuation Failure 需要遍历所有被修改的引用（需要一致的堆状态）；这些操作必须在 STW 中完成
- **为什么 Evacuation 期间也会产生脏卡？** 对象复制时，新对象的引用字段可能指向其他 Region 的对象（跨 Region 引用）；写屏障会标记这些卡为 dirty；Post-Evacuate Cleanup 处理这些脏卡，更新 RSet
