...
G1RootProcessor 的本质是**GC Roots 的并行扫描协调器**：将所有 GC Roots（线程栈/JNI/CLDG/CodeCache/静态变量等）分配给多个 GC Worker 并行扫描；每个 Worker 调用对应的 `OopClosure`（由 `G1RootClosures` 提供）处理每个 Root 引用。

### 0.2 为什么需要？

GC Roots 包括所有线程的栈帧（可能有数百个线程）、JNI 全局引用、类加载器数据图（CLDG）、CodeCache 中的引用等。串行扫描代价高（数百个线程的栈帧）；并行扫描让每个 GC Worker 负责一部分 Roots，线性加速。

### 0.3 怎么解决？

**任务分配 + 并行执行**：`G1RootProcessor::evacuate_roots()` 将 Root 扫描任务分配给 `WorkGang`；每个 GC Worker 执行 `G1EvacuationTask`，依次扫描：线程栈（每个 Worker 负责一部分线程）、JNI 全局引用、CLDG、CodeCache；扫描到的引用对象通过 `G1CopyingClosure` 复制到 Survivor/Old。

### 0.4 为什么这样设计？

- **为什么线程栈扫描可以并行？** 每个线程的栈帧是独立的，不同 Worker 扫描不同线程的栈帧，没有竞争；`Threads::possibly_parallel_oops_do()` 实现线程级别的并行分配
- **为什么 CLDG 扫描需要特殊处理？** CLDG（ClassLoaderDataGraph）包含所有类加载器的元数据，其中的引用（如静态变量）需要特殊的 `CLDClosure` 处理；类加载器本身也可能是 GC Root（如果类加载器是强可达的）
