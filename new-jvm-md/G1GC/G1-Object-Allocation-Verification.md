...
本文是 G1 对象分配路径的**GDB 验证报告**：通过实际运行 JVM 并设置 GDB 断点，验证 TLAB 分配路径、Eden Region 分配路径、Humongous 对象分配路径的实际执行情况；提供关键数据结构的实际值（TLAB 大小、Eden Region 地址等）。

### 0.2 验证目标

| 验证项 | 验证方法 | 预期结果 |
|--------|---------|---------|
| TLAB 大小 | GDB `p _tlab._end - _tlab._start` | 约 256KB（自适应） |
| Eden Region 地址 | GDB `p _mutator_alloc_region._alloc_region` | 有效 HeapRegion 指针 |
| Humongous 分配触发 | GDB 断点 `humongous_obj_allocate` | 对象 > 2MB 时触发 |
| TLAB 满时行为 | GDB 断点 `allocate_new_tlab` | TLAB 满时申请新 TLAB |

### 0.3 关键验证结论

- TLAB 默认大小约 256KB，自适应调整（`-XX:TLABSize` 可固定）
- Eden Region 满时触发 Young GC（`attempt_allocation_slow()` 返回 NULL）
- Humongous 对象（> 2MB）直接分配到 Humongous Region，不经过 TLAB
