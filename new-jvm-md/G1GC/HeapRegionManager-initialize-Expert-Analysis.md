...
`HeapRegionManager::initialize()` 的本质是**建立 Region 元数据管理体系**：创建 2048 个 `HeapRegion` 对象（每个 Region 的元数据），建立地址→Region 的 O(1) 映射（`_regions` BiasedMappedArray），初始化 Free Region 链表（`_free_list`），关联 6 个辅助数据结构映射器（BOT/CardTable/Bitmap 等）。

### 0.2 为什么需要？

G1 将堆切分为 2048 个 Region，每个 Region 有独立的元数据（类型/RSet/标记信息等）。需要一个中央管理器来：(1) 快速定位任意地址所属的 Region；(2) 管理空闲 Region 列表；(3) 堆扩展时同步 commit 6 个数据结构。`initialize()` 建立这个管理体系。

### 0.3 怎么解决？

**BiasedMappedArray + FreeRegionList**：`_regions` 是 `G1BiasedMappedArray<HeapRegion*>`，通过 `(addr - heap_start) >> log2_region_size` 直接索引；`_free_list` 是双向链表，管理所有 Free Region；`initialize()` 创建 2048 个 `HeapRegion` 对象并加入 `_free_list`。

### 0.4 为什么这样设计？

- **为什么用 BiasedMappedArray 而不是普通数组？** Biased 意味着数组基地址被偏移，使得 `array[heap_addr >> shift]` 直接得到 Region 指针，省去减法运算，在 GC 热路径上节省一条指令
- **为什么空闲列表用双向链表？** 双向链表支持 O(1) 的任意位置删除（Region 回收时需要从链表中间删除），栈/队列只支持头尾操作
