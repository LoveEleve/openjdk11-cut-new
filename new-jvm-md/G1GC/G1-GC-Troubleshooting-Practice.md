...
本文是 G1 GC 的**实战排查指南**：通过典型生产问题案例（Full GC 频繁/停顿超标/内存泄漏/Humongous 对象过多），展示如何用 GC 日志、GDB、JVM 参数诊断和解决 G1 GC 问题。

### 0.2 常见问题分类

| 问题类型 | 症状 | 根因 | 解决方案 |
|---------|------|------|---------|
| Full GC 频繁 | 停顿秒级，频率高 | Evacuation Failure / 并发标记失败 | 降低 IHOP，增大堆 |
| 停顿超标 | 单次停顿 > MaxGCPauseMillis | CSet 太大 / RSet 更新慢 | 减小年轻代，调整 Refinement |
| 内存泄漏 | 堆持续增长 | 对象无法被回收 | 堆 Dump 分析 |
| Humongous 对象 | 大量 Humongous Region | 大对象频繁分配 | 增大 Region 大小 |
...
1. **看 GC 日志**：确认 GC 类型（Young/Mixed/Full）和停顿时间
2. **看 GC 原因**：`GCCause::_g1_evacuation_pause` / `_g1_humongous_allocation` 等
3. **看 RSet 统计**：`-Xlog:gc+remset=trace` 查看 RSet 更新开销
4. **GDB 验证**：对特定问题设断点，验证假设
