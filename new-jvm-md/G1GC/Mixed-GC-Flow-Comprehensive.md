...
Mixed GC 完整流程的本质是**并发标记周期 + 空间回收阶段的两阶段协作**：并发标记周期（Initial Mark → Root Region Scan → Concurrent Mark → Remark → Cleanup）为 Mixed GC 提供 Old Region 存活率数据；空间回收阶段（多次 Mixed GC）利用这些数据回收垃圾密度高的 Old Region。

### 0.2 完整时序

```
Young GC（搭便车 Initial Mark）
    ↓
Root Region Scan（并发，扫描 Survivor Region）
    ↓
Concurrent Mark（并发，多线程标记对象图）
    ↓
Remark（STW，处理 SATB 队列）
    ↓
Cleanup（STW，统计存活字节，更新 CollectionSetChooser）
    ↓
Mixed GC × N（STW，Young + 部分 Old，直到 Old 区占用降低）
    ↓
回到 Young GC 模式（等待下次并发标记触发）
```
...
| 状态 | 触发条件 | 退出条件 |
|------|---------|---------|
| Young GC Only | 默认 | Old 区占用 > IHOP |
| Concurrent Marking | Old 区占用 > IHOP | Cleanup 完成 |
| Mixed GC | Cleanup 完成 | Old 区占用 < G1HeapWastePercent |
| Full GC | Evacuation Failure | 完成 Mark-Compact |

### 0.4 为什么这样设计？

- **为什么并发标记和 Mixed GC 分开两个阶段？** 并发标记需要时间（可能数秒），不能在 Mixed GC 的 STW 内完成；分开两个阶段让标记在后台并发进行，Mixed GC 只需使用标记结果
- **为什么 Mixed GC 要连续多次而不是一次完成？** 一次 Mixed GC 受停顿时间限制，只能回收部分 Old Region；多次 Mixed GC 逐步将 Old 区占用率降到目标以下
