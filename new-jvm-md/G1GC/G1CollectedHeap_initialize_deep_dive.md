...
`G1CollectedHeap::initialize()` 的本质是 **G1 GC 的"大爆炸"函数**：约 400 行代码，从零开始建立完整的 G1 内存管理体系——预留虚拟内存、创建 6 个数据结构映射器、初始化 2048 个 Region、建立卡表/屏障集/RSet、启动并发标记和精化线程。

### 0.2 为什么需要？

JVM 启动时 G1 GC 的所有子系统都需要初始化，且有严格的依赖顺序：卡表必须在 RSet 之前创建，HeapRegionManager 必须在 Region 分配之前初始化，并发标记线程必须在堆扩展之后启动。`initialize()` 按正确顺序完成所有初始化。

### 0.3 14 个阶段

1. 获取堆大小参数（`-Xms`/`-Xmx`）
2. 虚拟内存预留（`mmap(PROT_NONE)`）
3. 卡表和屏障集（`new G1CardTable/G1BarrierSet`）
4. 六大映射器（`G1RegionToSpaceMapper`）
5. HeapRegionManager（`_hrm.initialize()`）
6. G1RemSet（`new G1RemSet()`）
7. 快速测试数组（`_in_cset_fast_test`）
8. 并发标记（`new G1ConcurrentMark()`）
9. 堆扩展（`expand(init_byte_size)`）
10. 策略和队列（SATB/DirtyCard 队列）
11. 分配器（Dummy Region + mutator alloc）
12. 监控和去重（`new G1MonitoringSupport()`）
13. PreservedMarksSet
14. CollectionSet

### 0.4 为什么这样设计？

- **为什么虚拟内存预留在卡表创建之前？** 卡表大小 = 堆大小 / 512，必须先知道堆的虚拟地址范围
- **为什么六大映射器在 HeapRegionManager 之前？** HeapRegionManager 的 `initialize()` 需要传入这 6 个映射器
