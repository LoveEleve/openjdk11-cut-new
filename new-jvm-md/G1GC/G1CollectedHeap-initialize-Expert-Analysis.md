...
- **为什么虚拟内存预留（Phase 2）在卡表创建（Phase 3）之前？** 卡表大小 = 堆大小 / 512，必须先知道堆的虚拟地址范围才能创建卡表
- **为什么六大映射器（Phase 4）在 HeapRegionManager（Phase 5）之前？** HeapRegionManager 的 `initialize()` 需要传入这 6 个映射器作为参数
- **为什么 Dummy Region（Phase 11）是必要的？** `G1AllocRegion` 的 `_alloc_region` 字段永不为 NULL，Dummy Region 作为占位符，分配时必定失败，触发申请新 Region 的逻辑
