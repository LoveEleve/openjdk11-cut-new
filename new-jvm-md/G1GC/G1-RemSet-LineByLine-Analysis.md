...
`G1RemSet` 的本质是**RSet 更新和扫描的协调器**：`refine_card_concurrently()` 处理脏卡（扫描卡内引用，更新目标 Region 的 RSet）；`scan_rem_set()` 在 GC 时扫描 CSet 中每个 Region 的 RSet（找到所有跨 Region 引用，确保不漏标）；`update_rem_set()` 在 GC 开始时处理所有残留脏卡。

### 0.2 两个核心操作

| 操作 | 时机 | 方法 | 作用 |
|------|------|------|------|
| RSet 更新 | 并发（Refinement 线程） | `refine_card_concurrently()` | 脏卡→RSet 条目 |
| RSet 扫描 | GC STW | `scan_rem_set()` | RSet 条目→扫描引用 |

### 0.3 `refine_card_concurrently()` 流程

1. 清除卡的 dirty 标记（先清除，避免丢失新的 dirty）
2. 检查卡所在 Region 是否在 CSet（CSet 中的 Region 不需要更新 RSet）
3. 扫描卡内所有引用字段
4. 对于跨 Region 的引用，更新目标 Region 的 RSet（`HeapRegionRemSet::add_reference()`）

### 0.4 为什么这样设计？

- **为什么先清除 dirty 再扫描？** 如果先扫描再清除，扫描期间产生的新 dirty 会被清除，导致漏处理；先清除后，扫描期间产生的新 dirty 会在下一轮处理
- **为什么 CSet 中的 Region 不需要更新 RSet？** CSet 中的 Region 在本次 GC 中会被回收，其 RSet 不再有用；跳过 CSet Region 的 RSet 更新节省时间
