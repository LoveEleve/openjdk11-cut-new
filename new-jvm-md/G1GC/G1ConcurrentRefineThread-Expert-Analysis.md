...
G1ConcurrentRefineThread 的本质是**脏卡队列的单个消费者线程**：从全局 `DirtyCardQueueSet` 取出脏卡缓冲区，调用 `G1RemSet::refine_card_concurrently()` 处理每张脏卡（扫描卡内引用，更新目标 Region 的 RSet）；处理完后等待新的脏卡缓冲区。

### 0.2 为什么需要？

写屏障产生的脏卡需要被处理（转化为 RSet 条目），但不能在写屏障中同步处理（代价太高）。G1ConcurrentRefineThread 在后台并发处理，与应用线程并发运行，不需要 STW。

### 0.3 怎么解决？

**工作循环 + 激活/停用**：线程主循环：`try_refinement_step()` 尝试从队列取一个缓冲区处理；队列空时 `wait_for_completed_buffers()` 等待；`G1ConcurrentRefine` 根据队列积压量决定激活/停用线程（`activate()`/`deactivate()`）。

### 0.4 为什么这样设计？

- **为什么线程可以被停用而不是一直运行？** 低负载时脏卡少，多个 Refinement 线程空转浪费 CPU；停用空闲线程节省 CPU，需要时再激活（激活代价低，只需唤醒等待的线程）
- **为什么处理脏卡要"先清除再扫描"？** 先清除脏卡标记，再扫描卡内引用；如果扫描期间有新的写操作再次标记同一张卡，清除后这个新的 dirty 不会丢失（会被下一轮处理）
