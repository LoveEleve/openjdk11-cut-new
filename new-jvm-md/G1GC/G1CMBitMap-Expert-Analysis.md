...
G1CMBitMap 的本质是**堆对象存活状态的位图索引**：每个堆地址对应 1 位，标记该地址的对象是否存活；G1 维护两个 Bitmap（`_prev_mark_bitmap` 和 `_next_mark_bitmap`），形成双缓冲：`_next` 记录当前标记结果，`_prev` 记录上次标记结果（供 Mixed GC 使用）。

### 0.2 为什么需要？

并发标记需要记录哪些对象存活，但不能修改对象头（应用线程正在使用）。Bitmap 提供了独立的存活标记空间：每个对象对应 1 位，8GB 堆只需 128MB Bitmap（1/64 压缩比）；标记操作是原子的（CAS 设置位），不影响对象头。

### 0.3 怎么解决？

**双缓冲 + 地址→位索引**：给定对象地址 `addr`，位索引 = `(addr - heap_start) / 8`（每 8 字节对应 1 位，因为对象最小 8 字节对齐）；`mark_next(addr)` 原子设置 `_next_mark_bitmap` 中对应位；Cleanup 阶段 `swap_mark_bitmaps()` 交换双缓冲。

### 0.4 为什么这样设计？

- **为什么需要双缓冲而不是单个 Bitmap？** 并发标记期间 Mixed GC 可能同时运行，Mixed GC 需要读 `_prev_mark_bitmap`（上次标记结果）；如果只有一个 Bitmap，并发标记写入会破坏 Mixed GC 正在读的数据；双缓冲让两者互不干扰
- **为什么位图粒度是 8 字节（而不是 1 字节）？** Java 对象最小 8 字节对齐（64 位 JVM），8 字节粒度不会漏标任何对象；1 字节粒度会使 Bitmap 大 8 倍（1GB vs 128MB）
- **为什么标记操作用 CAS 而不是普通写？** 多个 GC 线程并发标记，可能同时标记同一个对象；CAS 保证只有一个线程成功标记，避免重复处理
