...
G1 Full GC 的本质是**G1 的兜底机制**：当 Evacuation Failure（Survivor/Old 空间不足）或并发标记来不及完成（堆满）时触发；使用串行 Mark-Compact 算法（`G1FullCollector`）对整个堆进行压缩，消除碎片，代价是长时间 STW（秒级）。

### 0.2 为什么需要？

G1 的正常 GC（Young GC/Mixed GC）是 Copying GC，需要目标 Region 有足够空间。当空间不足时（Evacuation Failure），无法完成 Copying GC；此时必须退化为 Full GC，用 Mark-Compact 整理整个堆，释放碎片空间。

### 0.3 四阶段 Full GC

1. **Mark**：标记所有存活对象（STW，多线程并行）
2. **Prepare Compaction**：计算每个对象的目标地址（压缩后的位置）
3. **Adjust Pointers**：更新所有引用指向新地址
4. **Compact**：将对象移动到目标地址

### 0.4 为什么这样设计？

- **为什么 G1 Full GC 用 Mark-Compact 而不是 Copying？** Copying GC 需要额外的空间（目标 Region），Full GC 触发时空间已经不足；Mark-Compact 原地压缩，不需要额外空间
- **为什么 Full GC 是串行的（G1FullCollector）？** Full GC 是兜底机制，设计目标是"能完成"而不是"快速完成"；串行实现简单可靠，避免并行 Mark-Compact 的复杂性
