...
G1RedirtyCardsClosure 的本质是**Evacuation 后 RSet 一致性修复器**：Evacuation 期间，对象被复制到新 Region，新对象的引用字段可能指向其他 Region（跨 Region 引用）；这些新的跨 Region 引用需要被记录到 RSet 中；G1RedirtyCardsClosure 将包含这些新引用的卡重新标记为 dirty，供 Concurrent Refinement 处理。

### 0.2 为什么需要？

Evacuation 期间，GC Worker 复制对象时会产生新的跨 Region 引用（新对象在 Survivor/Old Region，其引用字段指向其他 Region 的对象）。这些新引用需要更新 RSet，否则下次 GC 会漏标跨 Region 引用。G1RedirtyCardsClosure 将这些卡重新标记为 dirty，让 Concurrent Refinement 在 GC 后处理。

### 0.3 怎么解决？

**遍历 PLAB + 重新标记**：每个 GC Worker 的 PLAB 记录了本次 Evacuation 分配的所有对象；`G1RedirtyCardsClosure` 遍历这些对象，检查其引用字段是否跨 Region；如果跨 Region，将包含该字段的卡重新标记为 dirty（`card_table->dirty(addr)`）。

### 0.4 为什么这样设计？

- **为什么不在 Evacuation 时直接更新 RSet？** Evacuation 时直接更新 RSet 需要在 GC Worker 的热路径上做复杂的 RSet 操作（代价高）；重新标记脏卡代价低（只需写一个字节），RSet 更新推迟到 Concurrent Refinement
- **为什么叫 "Redirty"（重新标记脏）？** Evacuation 开始前，Concurrent Refinement 已经处理了所有脏卡（`update_rem_set()`）；Evacuation 期间产生的新跨 Region 引用需要"重新"标记为 dirty
