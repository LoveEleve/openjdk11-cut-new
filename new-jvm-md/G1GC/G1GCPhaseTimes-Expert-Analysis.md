...
G1GCPhaseTimes 的本质是**GC 停顿期间各阶段耗时的精确记录器**：维护 30+ 个阶段的计时数据（每个阶段有 per-worker 数据 + 汇总数据），GC 结束后生成结构化的 GC 日志（`-Xlog:gc+phases=debug`），同时为 `G1Policy` 的停顿预测提供历史数据。

### 0.2 为什么需要？

G1 的停顿预测需要精确的历史数据：每个阶段（Root 扫描/RSet 更新/对象复制等）的耗时是预测下次停顿时间的基础。同时，GC 调优需要知道停顿时间花在哪里（是 Root 扫描慢还是对象复制慢）。G1GCPhaseTimes 提供这两方面的数据。

### 0.3 怎么解决？

**WorkerDataArray + 层次化计时**：每个阶段用 `WorkerDataArray<double>` 记录每个 GC Worker 的耗时；`record_time_secs()` 记录单个 Worker 的时间；`sum_thread_work_items()` 汇总所有 Worker 的数据；`print_phases()` 生成层次化的 GC 日志。

### 0.4 为什么这样设计？

- **为什么需要 per-worker 数据而不是只记录总时间？** per-worker 数据可以发现负载不均（某个 Worker 特别慢）；总时间只能说明"慢了"，per-worker 数据能说明"哪个 Worker 慢了"，便于诊断
- **为什么用 `WorkerDataArray<double>` 而不是简单的 `double[]`？** `WorkerDataArray` 提供了 min/max/avg/sum 等统计方法，以及格式化输出；封装减少了重复代码
