...
本文是 `G1CollectedHeap::initialize()` 的**逐行深度分析（Part 2）**，覆盖 Lines 1745-2005：六大内存映射器创建（BOT/CardTable/Bitmap/Region 等）、HeapRegionManager 初始化（2048 个 Region 元数据）、G1RemSet 创建、快速测试数组初始化。

### 0.2 Part 2 覆盖范围

| 行号范围 | 阶段 | 核心操作 |
|---------|------|---------|
| 1745-1820 | 六大映射器 | 创建 6 个 `G1RegionToSpaceMapper`（BOT/CardTable/Bitmap 等） |
| 1821-1900 | HeapRegionManager | `_hrm.initialize(mappers...)` 创建 2048 个 HeapRegion |
| 1901-1950 | G1RemSet | `new G1RemSet()` 创建 RSet 管理器 |
| 1951-2005 | 快速测试数组 | `_in_cset_fast_test.initialize()` O(1) CSet 查询 |

### 0.3 六大映射器的作用

| 映射器 | 作用 |
|--------|------|
| BOT mapper | 块偏移表，每 Region 512 字节 |
| CardTable mapper | 卡表，每 Region 8192 字节 |
| Bitmap mapper | 标记位图，每 Region 64KB |
| Region mapper | Region 元数据，每 Region 固定大小 |
| Prev/Next Bitmap | 双缓冲标记位图 |
