...
G1ConcurrentRefine 的本质是**脏卡队列的弹性消费者管理器**：管理 1-4 个 `G1ConcurrentRefineThread`，根据 `DirtyCardQueueSet` 的积压量动态激活/停用线程；三色水位线（Green/Yellow/Red）控制线程数量和应用线程参与度，实现自适应的 RSet 更新速率。

### 0.2 为什么需要？

脏卡产生速率随应用负载动态变化：低负载时少量脏卡，高负载时大量脏卡。固定线程数在低负载时浪费 CPU，高负载时处理不及（积压增加 GC 停顿）。G1ConcurrentRefine 的弹性线程池自适应：负载低时少用线程，负载高时多用线程甚至让应用线程参与。

### 0.3 怎么解决？

**三色水位线 + 弹性线程**：
- 队列 < Green（13 缓冲区）：只有 1 个 Refinement 线程运行
- 队列 > Yellow（39 缓冲区）：激活更多 Refinement 线程（最多 4 个）
- 队列 > Red（65 缓冲区）：应用线程在分配时也帮助处理脏卡（背压）
- GC 开始时：`update_rem_set()` 处理所有残留脏卡

### 0.4 为什么这样设计？

- **为什么水位线是 13/39/65 而不是其他值？** 这些是经验值，基于典型应用的脏卡产生速率和 Refinement 线程处理能力；可通过 `-XX:G1ConcRefinementGreenZone` 等参数调整
- **为什么 Red 区让应用线程参与？** 如果 Refinement 线程跟不上，队列无限增长，GC 开始时需要处理大量积压脏卡，延长 STW；Red 区背压限制积压量，以牺牲少量应用吞吐换取 GC 停顿可控
