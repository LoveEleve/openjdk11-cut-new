...
`G1ConcurrentMark` 的本质是**并发标记周期的总控制器**：管理双缓冲 Bitmap（`_prev_mark_bitmap`/`_next_mark_bitmap`，各 128MB）、`G1CMTask` 数组（每个 GC Worker 一个）、全局溢出栈（`G1CMMarkStack`）；驱动 Initial Mark → Root Region Scan → Concurrent Mark → Remark → Cleanup 五个阶段。

### 0.2 核心字段

| 字段 | 类型 | 作用 |
|------|------|------|
| `_prev_mark_bitmap` | `G1CMBitMap*` | 上次标记结果（供 Mixed GC 使用） |
| `_next_mark_bitmap` | `G1CMBitMap*` | 本次标记结果（并发标记写入） |
| `_tasks` | `G1CMTask**` | 每个 GC Worker 的标记任务 |
| `_global_mark_stack` | `G1CMMarkStack` | 全局溢出栈 |
| `_finger` | `HeapWord*` | 并发标记的全局进度指针 |
| `_concurrent_marking_in_progress` | `bool` | 并发标记是否进行中 |

### 0.3 为什么需要双缓冲 Bitmap？

并发标记期间 Mixed GC 可能同时运行，Mixed GC 需要读 `_prev_mark_bitmap`（上次标记结果）；如果只有一个 Bitmap，并发标记写入会破坏 Mixed GC 正在读的数据；双缓冲让两者互不干扰；Cleanup 阶段 `swap_mark_bitmaps()` 交换双缓冲。
