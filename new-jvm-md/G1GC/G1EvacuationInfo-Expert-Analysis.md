...
G1EvacuationInfo 的本质是**单次 Evacuation 阶段的统计快照**：记录本次 Evacuation 回收了多少 Region、复制了多少字节、分配了多少 Survivor/Old Region 等关键指标；供 `G1Policy` 更新预测模型和 GC 日志输出使用。

### 0.2 为什么需要？

`G1Policy` 需要历史 GC 数据来预测下次 GC 的代价（EMA 预测模型）。每次 Evacuation 完成后，需要将本次的统计数据（复制字节数、Region 数量等）更新到预测模型。G1EvacuationInfo 是这些统计数据的载体。

### 0.3 怎么解决？

**轻量值对象**：G1EvacuationInfo 是一个简单的值对象（POD），在 Evacuation 开始时创建，各阶段填充字段，Evacuation 结束时传给 `G1Policy::record_collection_pause_end()` 更新预测模型。

### 0.4 为什么这样设计？

- **为什么用独立的 EvacuationInfo 而不是直接更新 G1Policy？** 解耦统计收集和策略更新：Evacuation 阶段只负责填充数据，策略更新在 Evacuation 完成后统一进行；便于测试和调试（可以单独检查 EvacuationInfo 的内容）
