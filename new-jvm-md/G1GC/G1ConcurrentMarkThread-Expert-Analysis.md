...
G1ConcurrentMarkThread 的本质是**并发标记周期的驱动线程**：一个后台守护线程，在 Initial Mark（STW）完成后被唤醒，依次驱动 Root Region Scan → Concurrent Mark → Remark（STW）→ Cleanup（STW）→ Concurrent Cleanup 五个阶段；每个阶段完成后等待下一次触发。

### 0.2 为什么需要？

并发标记的各阶段需要一个协调者：(1) 知道何时开始（Initial Mark 完成后）；(2) 按正确顺序执行各阶段；(3) 在需要 STW 的阶段（Remark/Cleanup）请求 SafePoint；(4) 在并发阶段与应用线程并发运行。G1ConcurrentMarkThread 就是这个协调者。

### 0.3 怎么解决？

**状态机 + 条件变量**：`G1ConcurrentMarkThread` 继承 `ConcurrentGCThread`，主循环等待 `_started` 信号；Initial Mark 完成后 `G1CollectedHeap` 调用 `start_marking_cycle()` 发送信号；线程依次执行各阶段，STW 阶段通过 `VM_G1Concurrent` VM Operation 请求 SafePoint。

### 0.4 为什么这样设计？

- **为什么用独立线程而不是在 VMThread 中执行？** 并发标记的大部分工作（Concurrent Mark）需要与应用线程并发运行，不能在 VMThread 中执行（VMThread 执行时所有应用线程都停止）；只有 Remark/Cleanup 需要 STW，通过 VM Operation 请求
- **为什么 Root Region Scan 必须在下次 Young GC 之前完成？** Root Region Scan 扫描 Survivor Region（上次 Young GC 的幸存者），这些 Region 在下次 Young GC 时会被回收；如果 Root Region Scan 未完成就触发 Young GC，会丢失标记信息
