...
G1AllocRegion 的本质是**GC 线程的 Region 级分配缓冲管理器**：维护一个当前活跃的分配 Region（`_alloc_region`），GC 线程从中分配 PLAB；当前 Region 满时申请新 Region；通过 Dummy Region 避免 NULL 检查，简化分配路径。

### 0.2 为什么需要？

Evacuation 阶段多个 GC 线程并发复制对象，需要向 Survivor/Old Region 分配内存。如果每次分配都加全局锁，锁竞争严重。G1AllocRegion 提供 Region 级别的分配管理：每个 GC 线程有自己的 PLAB（从 G1AllocRegion 分配），PLAB 内分配无锁。

### 0.3 怎么解决？

**Dummy Region + 懒惰申请**：`_alloc_region` 初始化为 Dummy Region（`_top == _end`，分配必定失败）；分配失败时调用 `new_alloc_region()` 申请真实 Region；Region 满时 `retire()` 将其变为 Survivor/Old，再申请新 Region。

### 0.4 为什么这样设计？

- **为什么用 Dummy Region 而不是 NULL 检查？** 每次分配都检查 `_alloc_region != NULL` 代价高（热路径上的额外分支）；Dummy Region 让分配路径统一（总是尝试分配，失败时走慢路径），消除 NULL 检查
- **为什么 G1AllocRegion 有 Survivor 和 Old 两个子类？** Survivor Region 有年龄限制（对象年龄 ≥ 阈值时晋升 Old），Old Region 没有；两个子类的 `new_alloc_region()` 逻辑不同
