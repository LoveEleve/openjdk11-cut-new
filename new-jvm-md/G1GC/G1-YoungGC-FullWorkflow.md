...
G1 Young GC 的本质是**Eden 区满时触发的 STW 并行 Copying GC**：将 Eden + Survivor Region 中的存活对象复制到新的 Survivor/Old Region，原 Region 整体释放；整个过程 STW，多个 GC Worker 并行执行，通常 < 200ms。
...
Eden 满 → attempt_allocation_slow() 失败
    ↓
VM_G1CollectForAllocation（VM Operation）
    ↓ STW 开始
G1CollectedHeap::do_collection_pause_at_safepoint()
    ↓
1. Pre-Evacuate：更新 RSet（处理残留脏卡）
2. Evacuate Collection Set：
   a. Root 扫描（G1RootProcessor）
   b. 传递闭包（G1ParEvacuateFollowersClosure）
3. Post-Evacuate：释放 CSet Region，更新统计
    ↓ STW 结束
G1Policy::record_collection_pause_end()（更新预测模型）
```
...
| 阶段 | 典型时间 | 主要工作 |
|------|---------|---------|
| Pre-Evacuate | 5-20ms | 处理残留脏卡，更新 RSet |
| Root 扫描 | 5-30ms | 扫描线程栈/JNI/CLDG 等 |
| 对象复制 | 50-150ms | 并行复制存活对象 |
| Post-Evacuate | 5-20ms | 释放 CSet Region，更新统计 |

### 0.4 为什么这样设计？

- **为什么 Young GC 必须 STW？** Copying GC 需要移动对象并更新所有引用，不能在应用线程运行时进行（引用更新不是原子的）
- **为什么 Young GC 通常很短（< 200ms）？** Young 区通常只有堆的 5-60%；存活对象少（大多数 Eden 对象是短命的）；多线程并行复制
