...
Initial Mark Phase 的本质是**并发标记周期的 STW 起点**：在 Young GC 的 STW 中"搭便车"，完成两件事：(1) 标记 GC Roots 直接可达的对象（`_next_mark_bitmap`）；(2) 设置每个 Region 的 `_next_top_at_mark_start`（TAMS），记录标记开始时的 `_top`，保护标记期间新分配的对象。

### 0.2 为什么需要？

并发标记需要一个一致的起点：知道哪些对象是"标记开始时就存活的"（GC Roots 直接可达），以及哪些对象是"标记期间新分配的"（TAMS 以上）。Initial Mark 在 STW 中建立这个一致的起点，之后并发标记才能安全地并发运行。

### 0.3 怎么解决？

**搭便车 + TAMS 设置**：Initial Mark 搭便车在 Young GC 的 STW 中完成（`G1CollectorState::_in_initial_mark_gc = true`）；Young GC 的 Root 扫描阶段同时完成 Initial Mark 的标记工作；`G1ConcurrentMark::checkpoint_roots_initial_pre()` 设置所有 Region 的 TAMS。

### 0.4 为什么这样设计？

- **为什么 Initial Mark 搭便车在 Young GC 上？** Initial Mark 需要 STW，如果单独触发 STW 会增加一次额外停顿；Young GC 本来就需要 STW，搭便车不增加额外停顿次数，只是在 Young GC 的 STW 中额外做一些标记工作
- **为什么需要 TAMS？** 并发标记期间应用线程继续分配对象，这些新对象在 `_next_mark_bitmap` 中没有标记位；TAMS 记录标记开始时的 `_top`，TAMS 以上的对象是新分配的，默认存活，不需要标记
