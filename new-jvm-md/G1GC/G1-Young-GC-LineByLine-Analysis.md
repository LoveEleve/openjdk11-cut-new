...
本文是 G1 Young GC 的**逐行深度分析（Part 1）**，覆盖从 `VM_G1CollectForAllocation` 触发到 `do_collection_pause_at_safepoint()` 的完整流程：VM Operation 机制、SafePoint 同步、GC 前准备（RSet 更新/CSet 构建）。

### 0.2 Part 1 覆盖范围

| 阶段 | 源码位置 | 核心操作 |
|------|---------|---------|
| GC 触发 | `vm_operations_g1.cpp` | `VM_G1CollectForAllocation::doit()` |
| SafePoint 同步 | `safepoint.cpp` | 等待所有线程到达 SafePoint |
| GC 前准备 | `g1CollectedHeap.cpp:3541` | `do_collection_pause_at_safepoint()` |
| RSet 更新 | `g1RemSet.cpp` | `update_rem_set()` 处理残留脏卡 |
| CSet 构建 | `g1CollectionSet.cpp` | `finalize_initial_collection_set()` |

### 0.3 VM Operation 机制

Young GC 通过 `VM_G1CollectForAllocation` VM Operation 触发：应用线程调用 `VMThread::execute(&op)`，将 GC 请求提交给 VMThread；VMThread 在 SafePoint 执行 `op.doit()`（此时所有应用线程已停止）；GC 完成后 VMThread 释放 SafePoint，应用线程继续运行。
