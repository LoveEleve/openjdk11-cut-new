...
G1 预测模型的本质是**基于 EMA（指数移动平均）的自适应控制系统**：`G1Analytics` 维护 17 个 `TruncatedSeq`（截断历史序列），`G1Predictions` 从这些序列计算 EMA + 置信区间，`G1Policy` 用预测值做出决策（CSet 大小/IHOP/年轻代大小）。

### 0.2 EMA 计算公式

```
EMA(n) = α × sample(n) + (1-α) × EMA(n-1)
其中 α = 0.5（衰减因子）

预测值 = EMA + confidence_factor × 标准差
其中 confidence_factor = 1.645（95% 置信区间）
```
...
| 序列 | 含义 | 用途 |
|------|------|------|
| `_recent_gc_times_ms` | 最近 GC 停顿时间 | 预测下次停顿时间 |
| `_concurrent_mark_remark_times_ms` | Remark 时间 | 预测 Remark 停顿 |
| `_alloc_rate_ms` | 分配速率 | 计算自适应 IHOP |
| `_rs_length_diff_seq` | RSet 长度变化 | 预测 RSet 更新代价 |
| ... | ... | ... |

### 0.4 为什么这样设计？

- **为什么用 EMA 而不是简单平均？** EMA 让近期数据权重更高，能快速适应应用行为变化；简单平均对历史数据等权，响应变化慢
- **为什么预测值要加上标准差？** 加上标准差后，实际停顿超过预测值的概率降低到 5%（95% 置信区间）；G1 宁可少回收一些 Region，也不要停顿超标
