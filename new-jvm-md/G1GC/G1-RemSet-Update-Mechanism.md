...
G1 RSet 更新机制的本质是**写屏障→脏卡队列→并发精炼→RSet 的异步更新流水线**：应用线程写屏障只做最轻量的操作（将脏卡地址入线程本地队列），后台 Concurrent Refinement 线程从全局队列取出脏卡，扫描卡内引用，更新目标 Region 的 RSet。
...
应用线程写引用字段
    ↓ Post 写屏障（纳秒级）
将脏卡地址入 DirtyCardQueue（线程本地）
    ↓ 队列满时 flush
全局 DirtyCardQueueSet
    ↓ Concurrent Refinement 线程消费
refine_card_concurrently()
    ↓ 扫描卡内引用
HeapRegionRemSet::add_reference()
    ↓ 更新三级 RSet 结构
RSet 条目（Sparse/Fine/Coarse）
```

### 0.3 性能优化点

- **HotCardCache**：热卡去重，避免频繁修改的卡被重复处理
- **三色水位线**：弹性线程数，自适应处理速率
- **GC 前 update_rem_set()**：处理所有残留脏卡，确保 GC 时 RSet 完整

### 0.4 为什么这样设计？

- **为什么用异步更新而不是同步更新？** 同步更新（写屏障中直接更新 RSet）代价高（微秒级），严重影响应用吞吐；异步更新（写屏障只入队，后台线程处理）让写屏障代价降到纳秒级
- **为什么 GC 前需要 update_rem_set()？** GC 需要完整的 RSet 才能正确扫描跨 Region 引用；如果有残留脏卡未处理，GC 会漏标跨 Region 引用
