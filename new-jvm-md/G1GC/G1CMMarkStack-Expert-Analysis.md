...
G1CMMarkStack 的本质是**并发标记的全局溢出栈**：当 `G1CMTask` 的线程本地队列满时，将多余的对象引用 push 到全局 `G1CMMarkStack`；其他 Task 队列空时从 `G1CMMarkStack` pop 对象继续标记。它是并发标记 Work Stealing 机制的全局缓冲。

### 0.2 为什么需要？

并发标记时，每个 `G1CMTask` 有线程本地队列（`TaskQueue`，固定大小）。当某个 Task 处理的对象引用特别多（如大数组），本地队列可能溢出。需要一个全局溢出栈来接收溢出的引用，避免丢失标记任务。

### 0.3 怎么解决？

**分块链表 + 原子操作**：`G1CMMarkStack` 内部是一个分块链表（每块 `_chunk_capacity` 个槽位）；`push()` 原子地将对象引用写入当前块；块满时分配新块；`pop()` 从栈顶取对象；`is_empty()` 检查是否为空（用于判断标记是否完成）。

### 0.4 为什么这样设计？

- **为什么用分块链表而不是动态数组？** 动态数组扩容需要复制所有元素（代价高，且需要 STW）；分块链表只需分配新块，不需要复制，且可以并发操作
- **为什么 G1CMMarkStack 是全局的而不是每个 Task 独立？** 全局栈让所有 Task 共享溢出空间，避免某个 Task 溢出而其他 Task 空闲的情况；Work Stealing 从全局栈取任务，实现负载均衡
