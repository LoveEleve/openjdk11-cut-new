...
G1CMTask 的本质是**并发标记的单线程工作单元**：每个 GC 线程对应一个 G1CMTask，负责从任务队列取对象、扫描其引用字段、将未标记的引用对象加入队列并标记；多个 G1CMTask 并发运行，通过 Work Stealing 实现负载均衡。

### 0.2 为什么需要？

并发标记需要多线程并发遍历对象图。每个线程需要独立的工作状态（任务队列、统计信息、SATB 队列处理状态），避免线程间的状态竞争。G1CMTask 封装了单个标记线程的所有状态，多个 G1CMTask 并发运行互不干扰。

### 0.3 怎么解决？

**线程本地队列 + 时间片控制**：每个 G1CMTask 有独立的 `TaskQueue`（线程本地，无锁）；`do_marking_step()` 是核心方法，在时间片内尽量多标记对象；时间片到期时主动让出（`_has_timed_out = true`），让 GC 线程检查 SafePoint；队列空时尝试 Work Stealing 或从全局 `G1CMMarkStack` 取任务。

### 0.4 为什么这样设计？

- **为什么 `do_marking_step()` 有时间片限制？** 并发标记与应用线程并发运行，如果标记线程长时间不让出 CPU，会影响应用吞吐；时间片控制让标记线程定期检查 SafePoint，响应 Young GC 等 STW 请求
- **为什么线程本地队列用无锁设计？** 线程本地队列只有当前线程写入（push），其他线程只能从队列尾部 steal（Work Stealing 协议），无需锁；只有 steal 时需要 CAS
