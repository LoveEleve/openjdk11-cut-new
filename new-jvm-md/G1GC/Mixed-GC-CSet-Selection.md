...
Mixed GC CSet 选择的本质是**在停顿时间预算内，从 Old Region 候选集中贪心选取垃圾密度最高的 Region**：`CollectionSetChooser` 维护按垃圾密度（`reclaimable_bytes`）降序排列的 Old Region 候选列表；`G1Policy::finalize_old_part_of_collection_set()` 从中贪心选取，直到预测停顿时间达到 `MaxGCPauseMillis`。

### 0.2 为什么需要？

Mixed GC 的目标是在不超过停顿时间目标的前提下，尽量多回收 Old Region。如果随机选取 Old Region，可能选到存活率高的 Region（复制代价大，回收空间少）；按垃圾密度排序后贪心选取，在相同停顿时间内最大化回收空间。

### 0.3 CSet 选择流程

```
finalize_collection_set()
  ├── finalize_initial_collection_set()   // 加入所有 Young Region
  └── finalize_old_part_of_collection_set()  // Mixed GC 时加入 Old Region
        ├── 从 CollectionSetChooser 取候选 Old Region（按垃圾密度降序）
        ├── 预测加入该 Region 后的停顿时间增量
        ├── 如果总预测停顿时间 < MaxGCPauseMillis → 加入 CSet
        └── 否则停止选取
```

### 0.4 为什么这样设计？

- **为什么按垃圾密度而不是按 Region 大小排序？** 垃圾密度高的 Region 复制代价低（存活对象少）且回收空间大；按大小排序可能选到存活率高的大 Region，代价高收益低
- **为什么用贪心而不是动态规划？** 动态规划需要枚举所有 Region 组合（NP 问题），代价不可接受；贪心在实践中效果接近最优，且 O(n) 时间复杂度
