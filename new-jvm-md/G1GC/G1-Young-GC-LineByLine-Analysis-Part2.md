...
本文是 G1 Young GC 的**逐行深度分析（Part 2）**，覆盖 Evacuation 阶段到 GC 完成：`evacuate_collection_set()`（并行对象复制）、`post_evacuate_collection_set()`（释放 CSet Region/更新 RSet）、`record_collection_pause_end()`（更新预测模型）。

### 0.2 Part 2 覆盖范围

| 阶段 | 源码位置 | 核心操作 |
|------|---------|---------|
| Evacuation | `g1CollectedHeap.cpp:4789` | 并行复制存活对象 |
| Post-Evacuate | `g1CollectedHeap.cpp:5100` | 释放 CSet Region，更新 RSet |
| 统计更新 | `g1Policy.cpp` | `record_collection_pause_end()` |
| 堆扩展 | `g1CollectedHeap.cpp` | 如果需要，扩展堆 |

### 0.3 Evacuation 并行化

`evacuate_collection_set()` 使用 `WorkGang` 并行执行：每个 GC Worker 执行 `G1EvacuationTask`，依次完成 Root 扫描 + 传递闭包；Work Stealing 实现负载均衡；所有 Worker 完成后 `WorkGang::run_task()` 返回。

### 0.4 Post-Evacuate 关键操作

1. **释放 CSet Region**：将 CSet 中的 Region 加入 Free List（`_hrm.free_region()`）
2. **重新标记脏卡**：`G1RedirtyCardsClosure` 将 Evacuation 产生的新跨 Region 引用标记为 dirty
3. **处理 Evacuation Failure**：如果有对象无法复制，修复原地保留的对象的引用
4. **更新统计**：`G1EvacuationInfo` 传给 `G1Policy::record_collection_pause_end()`
