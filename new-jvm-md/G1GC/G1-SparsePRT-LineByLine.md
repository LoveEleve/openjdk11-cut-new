...
`SparsePRT` 的本质是**RSet 三级存储的第一层（最精确、最省内存）**：用哈希表存储 `(from_region_idx, card_idx[])` 对，每个 from_region 最多记录 4 张卡；当 from_region 数量超过阈值（默认 4）时，升级为 Fine 级别（`PerRegionTable`）。

### 0.2 数据结构

```cpp
struct SparsePRTEntry {
    RegionIdx_t _region_ind;  // from_region 索引（4 字节）
    CardIdx_t   _cards[4];    // 最多 4 张卡的索引（每个 2 字节）
    // 总大小：4 + 4×2 = 12 字节
};

class SparsePRT {
    SparsePRTEntry* _entries;  // 哈希表数组
    int _capacity;             // 哈希表容量（2 的幂次）
    int _num_entries;          // 当前条目数
};
```

### 0.3 哈希冲突处理

线性探测：`index = region_ind % capacity`；冲突时 `index = (index + 1) % capacity`；负载因子 > 0.75 时扩容（容量翻倍）。

### 0.4 为什么这样设计？

- **为什么每个 from_region 最多 4 张卡？** 4 张卡是经验值：大多数跨 Region 引用集中在少数几张卡；超过 4 张卡时升级为 Fine 级别（位图），查找更高效
- **为什么用线性探测而不是链表？** 线性探测的缓存局部性好（连续内存访问）；链表需要动态内存分配，代价高
