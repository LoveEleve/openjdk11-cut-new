...
Remark Phase 的本质是**并发标记的 STW 收尾阶段**：处理并发标记期间应用线程产生的 SATB 队列（Pre 写屏障记录的旧引用），确保"标记开始时存活的对象"全部被标记；同时处理弱引用（`WeakReference`/`SoftReference`/`PhantomReference`）的清除决策。

### 0.2 为什么需要？

并发标记（Concurrent Mark）期间，应用线程继续运行并修改引用（Pre 写屏障将旧引用入 SATB 队列）。这些 SATB 队列中的旧引用可能指向未被标记的对象（漏标风险）。Remark 在 STW 中处理所有 SATB 队列，确保没有漏标，然后才能进入 Cleanup 阶段。

### 0.3 怎么解决？

**STW + 并行 SATB 处理**：Remark 触发 STW，多个 GC Worker 并行处理 SATB 队列（`G1CMTask::drain_satb_buffers()`）；每个 SATB 队列中的旧引用被加入标记队列，继续标记传递闭包；所有 SATB 队列处理完后，标记完成（`_concurrent_mark_in_progress = false`）。

### 0.4 为什么这样设计？

- **为什么 Remark 必须 STW？** 处理 SATB 队列期间，如果应用线程继续运行，会产生新的 SATB 条目；STW 保证 SATB 队列处理完后不会有新的条目产生，标记结果是完整的
- **为什么 Remark 通常很短（< 100ms）？** 并发标记期间 SATB 队列的积累量有限（Pre 写屏障只记录被覆盖的旧引用，不是所有写操作）；Remark 只需处理这些增量，不需要重新扫描整个堆
- **为什么弱引用在 Remark 处理？** 弱引用的清除决策需要知道对象是否存活（标记结果）；Remark 完成后标记结果确定，可以决定哪些弱引用需要清除
