...
G1RegionMarkStats 的本质是**并发标记期间每个 Region 的存活字节统计缓存**：每个 GC Worker 有独立的 `G1RegionMarkStatsCache`（线程本地缓存），标记对象时累加存活字节；标记完成后 flush 到全局 `G1RegionMarkStats` 数组；Cleanup 阶段用这些数据计算每个 Region 的存活率。

### 0.2 为什么需要？

并发标记需要统计每个 Region 的存活字节数（供 Cleanup 阶段计算存活率，决定哪些 Region 值得 Mixed GC 回收）。如果每次标记一个对象都直接更新全局统计，需要原子操作（多线程并发标记），代价高。线程本地缓存减少原子操作频率。

### 0.3 怎么解决？

**线程本地缓存 + 批量 flush**：每个 `G1CMTask` 有独立的 `G1RegionMarkStatsCache`；标记对象时 `add_live_words(region_idx, words)` 更新线程本地缓存；缓存满时 `flush()` 将数据原子地合并到全局 `G1RegionMarkStats` 数组；标记完成后统一 flush 所有缓存。

### 0.4 为什么这样设计？

- **为什么需要线程本地缓存而不是直接更新全局数组？** 并发标记有多个 GC Worker 线程，直接更新全局数组需要原子操作（每次标记一个对象都需要 CAS），代价高；线程本地缓存批量更新，减少原子操作次数
- **为什么缓存大小有限制？** 缓存太大：flush 时需要处理大量数据，延迟高；缓存太小：频繁 flush，原子操作多；固定大小是内存和性能的平衡
