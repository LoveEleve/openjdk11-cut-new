...
| 根来源 | 处理方式 | 并行化 |
|--------|---------|--------|
| 线程栈帧 | `Threads::possibly_parallel_oops_do()` | ✅ 线程级并行 |
| JNI 全局引用 | `JNIHandles::oops_do()` | ✅ |
| CLDG（类加载器数据图） | `ClassLoaderDataGraph::oops_do()` | ✅ |
| CodeCache（nmethod） | `CodeCache::blobs_do()` | ✅ |
| Universe 根 | `Universe::oops_do()` | ❌ 串行 |
| SystemDictionary | `SystemDictionary::oops_do()` | ❌ 串行 |
| ... | ... | ... |

### 0.3 并行扫描机制

`G1RootProcessor::evacuate_roots()` 将 Root 扫描任务分配给 `WorkGang`；每个 GC Worker 执行 `G1EvacuationTask`，通过 `_lock` 和 `_claimed_*` 标志实现任务认领（避免重复扫描）；线程栈扫描通过 `Threads::possibly_parallel_oops_do()` 实现线程级并行。

### 0.4 为什么这样设计？

- **为什么线程栈扫描可以并行？** 每个线程的栈帧是独立的，不同 Worker 扫描不同线程的栈帧，没有竞争
- **为什么 Universe/SystemDictionary 是串行的？** 这些根数量少，并行化的同步开销大于收益；串行处理更简单高效
