...
`SurvRateGroup` 的本质是**年轻代 Region 存活率的历史统计器**：按 Region 年龄（在年轻代中经历的 GC 次数）分组，统计每个年龄组的历史存活率；`G1Policy` 用这些存活率预测下次 GC 时各年龄组的存活字节数，从而预测 Evacuation 的代价（需要复制多少字节）。

### 0.2 为什么需要？

`G1Policy` 预测 CSet 的 Evacuation 代价时，需要知道"回收一个 Eden Region 需要复制多少字节"。不同年龄的 Region 存活率不同（年龄越大，存活率越高，因为短命对象已经被回收）。`SurvRateGroup` 按年龄分组统计存活率，提供更精确的预测。

### 0.3 工作原理

每次 GC 后，`G1Policy::record_collection_pause_end()` 更新 `SurvRateGroup`：
- 统计每个年龄组的实际存活率（`survived_bytes / region_size`）
- 用 EMA 更新历史存活率（`_surv_rate_pred[age]`）
- `G1Policy` 用 `predict_surv_rate(age)` 预测下次 GC 的存活率

### 0.4 为什么这样设计？

- **为什么按年龄分组而不是统一平均？** 不同年龄的 Region 存活率差异很大（新 Eden Region 存活率低，老 Survivor Region 存活率高）；按年龄分组预测更精确
- **为什么用 EMA 而不是简单平均？** EMA 让近期数据权重更高，能快速适应应用行为变化
