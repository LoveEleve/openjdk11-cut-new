...
G1MMUTracker 的本质是**GC 停顿时间的滑动窗口守门员**：维护最近 `GCPauseIntervalMillis`（默认 201ms）时间窗口内的 GC 停顿历史，计算"当前时刻可以执行多长时间的 GC 而不违反 MMU 约束"；`G1Policy` 用这个值限制 GC 停顿时间。

### 0.2 MMU 的定义

MMU（Minimum Mutator Utilization）= 在任意 `GCPauseIntervalMillis` 时间窗口内，应用线程（Mutator）的最小运行时间比例。例如 MMU = 80% 意味着：在任意 201ms 窗口内，应用线程至少运行 160ms，GC 停顿最多 41ms。

### 0.3 工作原理

**滑动时间窗口 + 历史记录**：
- `G1MMUTracker` 维护一个环形缓冲区，记录最近的 GC 停顿（开始时间 + 持续时间）
- `when_sec(now, pause_time)` 计算"从 now 开始，执行 pause_time 的 GC 不会违反 MMU"的最早时刻
- `add_pause(start, end)` 在 GC 完成后更新历史记录

### 0.4 为什么这样设计？

- **为什么用滑动窗口而不是固定时间段？** 固定时间段有边界效应（窗口切换时统计重置）；滑动窗口任意时刻都能准确反映最近的 GC 负担
- **为什么 `GCPauseIntervalMillis` 默认 201ms？** 比 `MaxGCPauseMillis`（200ms）多 1ms，确保在最坏情况下应用线程至少有 1ms 的运行时间
