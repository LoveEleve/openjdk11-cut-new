...
本文是 G1 Young GC 的 **GDB 调试实战指南**：介绍如何用 GDB 断点追踪 Young GC 的完整执行流程，包括关键断点位置、调试技巧、常见问题排查方法。

### 0.2 调试 Young GC 的挑战

- **STW 期间调试**：Young GC 在 STW 期间执行，GDB 断点会暂停整个 JVM（包括 GC 线程）
- **多线程并行**：Evacuation 阶段多个 GC Worker 并行执行，需要区分线程
- **符号名复杂**：C++ 模板和命名空间导致符号名很长，需要用 `nm | grep` 确认

### 0.3 关键断点位置

| 断点 | 作用 | 触发时机 |
|------|------|---------|
| `G1CollectedHeap::do_collection_pause_at_safepoint` | Young GC 入口 | 每次 Young GC |
| `G1CollectedHeap::evacuate_collection_set` | Evacuation 开始 | Evacuation 阶段 |
| `G1ParScanThreadState::copy_to_survivor_space` | 对象复制 | 每个对象复制时 |
| `G1CollectedHeap::post_evacuate_collection_set` | Post-Evacuate | GC 收尾 |

### 0.4 GDB 调试最佳实践

- **使用脚本文件**：将断点和命令写入 `.gdb` 文件，避免手动输入
- **条件断点**：用 `break xxx if condition` 只在特定条件下停止
- **非停止模式**：用 `set non-stop on` 让其他线程继续运行
