...
ConcurrentHashTable 的本质是一个**读 Wait-Free、写 Lock-Free（快速路径）的并发哈希表**：读取线程通过 RCU（GlobalCounter）保护，无锁无重试；写入线程通过 CAS 头插，失败时自旋重试；删除通过桶级自旋锁串行化；扩容渐进式逐桶迁移，不阻塞读写。

### 0.2 为什么需要？

JVM 内部的 StringTable/SymbolTable/ThreadIdTable 有极端的**读多写少**特征：类加载时写入字符串常量，运行时大量读取。传统方案（全局读写锁/分段锁）在读取时仍有锁开销，无法满足 JVM 对低延迟的要求。同时 JVM 需要精确控制扩容时机（不能有全局停顿），普通 HashMap 的全量扩容不可接受。

### 0.3 怎么解决？

**三层并发控制**：
- **读取（Wait-Free）**：`GlobalCounter::critical_section_begin/end` 实现 RCU 语义，读取线程进入 Critical Section 后可安全访问，无锁无重试
- **写入（Lock-Free 快速路径）**：CAS 替换链表头（`cas_first`），失败时退避重试；桶被锁时 `yield()`，否则 `SpinPause()`
- **删除（桶级互斥）**：`trylock()` 获取桶级自旋锁，串行化同一桶的删除操作；删除后 `GlobalCounter::write_synchronize()` 等待所有读取者离开再释放内存

**渐进式扩容**：旧表 bucket 设置 REDIRECT 标志后，读写操作自动转向新表；`unzip_bucket()` 逐桶迁移，每次迁移一个节点后调 `write_synchronize_on_visible_epoch()` 确保读取者看到一致状态。

### 0.4 为什么这样设计？

- **为什么读取要 Wait-Free 而不是 Lock-Free？** JVM 内部表的读取在热路径上（类加载、字符串常量解析），需要稳定低延迟。Lock-Free 允许饥饿，Wait-Free 保证每个线程在有限步骤内完成，GC 停顿时间更可控
- **为什么用 RCU 而不是引用计数？** 引用计数需要每次读取都做原子 inc/dec，在高并发读取下竞争激烈；RCU 的 critical_section_begin/end 只需要写一个线程本地计数器，开销极低
- **为什么桶状态存在 `_first` 指针的低 2 位？** 桶的锁状态和链表头指针需要原子地一起修改（CAS），如果分开存储就需要两个原子操作，无法保证原子性；指针对齐保证低 2 位始终为 0，可以安全复用
- **为什么渐进式扩容而不是协助式扩容（CHM 风格）？** JVM 需要严格控制延迟，协助式扩容可能导致某个线程承担大量迁移工作；渐进式扩容将工作分散到每次访问，最坏延迟更低
...
```cpp
// concurrentHashTable.hpp:120
class Bucket {
 private:
  // ★ 核心字段：链表头指针 + 低 2 位状态
  // 低 2 位编码：00=UNLOCKED, 01=LOCKED, 10=REDIRECT
  Node* volatile _first;
  
  static const uintptr_t STATE_LOCK_BIT     = 0x1;  // bit 0: 锁定
  static const uintptr_t STATE_REDIRECT_BIT = 0x2;  // bit 1: 重定向（扩容中）
  static const uintptr_t STATE_MASK         = 0x3;  // 状态掩码
};
```

**sizeof(Bucket)**：**8 字节**（单个指针，GDB 验证：`p sizeof(ConcurrentHashTable<...>::Bucket)` = 8）

**创建位置**：`InternalTable` 构造时 `new Bucket[size]` 批量创建，从 C heap 分配（`MEMFLAGS F` 模板参数指定）。

**关键字段生命周期**：
- `_first`（UNLOCKED 状态）：`InternalTable` 创建时初始化为 NULL；`cas_first()` 时 CAS 替换为新节点；`release_assign_node_ptr()` 时 release-store 更新
- `_first`（LOCKED 状态）：`trylock()` 时 CAS 将 bit 0 置 1；`unlock()` 时 CAS 将 bit 0 清 0；持锁期间其他线程的 `cas_first()` 会失败（检测到 bit 0 = 1）
- `_first`（REDIRECT 状态）：`redirect()` 时 CAS 将 bit 1 置 1；这是**终态**，不可逆；读取者检测到 REDIRECT 后转向新表

### Node — 链表节点

```cpp
// concurrentHashTable.hpp:80
class Node : public CHeapObj<F> {
 private:
  Node* volatile _next;  // ★ 下一个节点（低 2 位始终为 0，对齐保证）
  VALUE _value;          // ★ 存储的值（模板参数，如 oop* 或 Symbol*）
};
```

**sizeof(Node)**：**16 字节**（ptr(8) + VALUE(8)，VALUE 通常是指针）

**创建位置**：`insert()` 快速路径中 `Node::create_node(value_f(), first_at_start)` 创建，从 `CHeapObj<F>` 分配（`new(F) Node(...)`）。

**关键字段生命周期**：
- `_next`：构造时设为 `first_at_start`（当前链表头）；`cas_first()` 成功后，此节点成为新链表头；`remove()` 时前驱节点的 `_next` 被 release-store 为 `this->_next`，此节点从链表摘除
- `_value`：构造时由 `value_f()` 函数对象填充；`node->value()` 返回 `&_value`；`GlobalCounter::write_synchronize()` 后 `Node::destroy_node()` 释放

### InternalTable — 内部哈希表（Bucket 数组容器）

```cpp
// concurrentHashTable.hpp:155
class InternalTable : public CHeapObj<F> {
 public:
  const size_t _log2_size;  // ★ log2(桶数量)，用于 hash & mask 计算
  const size_t _size;       // ★ 桶数量 = 1 << _log2_size
  Bucket       _buckets[1]; // ★ 变长数组（实际大小 = _size）
};
```

**sizeof(InternalTable)**：**24 + 8×size 字节**（2 个 size_t + 变长 Bucket 数组）

**创建位置**：`ConcurrentHashTable` 构造时 `new InternalTable(log2_size)` 创建初始表；`internal_grow_prolog()` 中创建新表（`_log2_size + 1`）；`internal_shrink_prolog()` 中创建新表（`_log2_size - 1`）。

**关键字段生命周期**：
- `_log2_size`/`_size`：构造时设置，不可变
- `_buckets[]`：构造时全部初始化为 NULL（UNLOCKED 空桶）；`unzip_bucket()` 迁移时逐桶填充；`internal_grow_epilog()` 后旧表被 `delete`，新表成为 `_table`
...
classDiagram
    class ConcurrentHashTable {
        ...
        -Mutex* _resize_lock
        -Thread* _resize_lock_owner
        -volatile InternalTable* _invisible_epoch
        +get(thread, lookup_f)
        +insert(thread, lookup_f, value_f)
        +remove(thread, lookup_f)
    }
    ...
    class Bucket {
        -Node* volatile _first
        低2位: 00=UNLOCKED 01=LOCKED 10=REDIRECT
        sizeof: 8 B
        +trylock()
        +unlock()
        +redirect()
    ...
    class GlobalCounter {
        +critical_section_begin(thread)
        +critical_section_end(thread)
        +write_synchronize()
        RCU 语义保护表结构
    }

    ConcurrentHashTable "1" --> "2" InternalTable : _table/_new_table
    InternalTable "1" --> "N" Bucket : _buckets[]
    Bucket "1" --> "0..N" Node : _first链表
    Node "1" --> "0..1" Node : _next
    ConcurrentHashTable ..> GlobalCounter : 读写同步
```
