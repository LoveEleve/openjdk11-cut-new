...
逃逸分析的本质是一个**基于连接图（Connection Graph）的流无关指向分析**：为每个 Java 对象（`AllocateNode`）和引用变量（`LocalVarNode`/`FieldNode`）建立指向关系图，通过不动点迭代传播逃逸状态（NoEscape → ArgEscape → GlobalEscape），最终判断哪些对象不逃逸方法或线程，从而做标量替换和锁消除。

### 0.2 为什么需要？

Java 的每个对象都分配在堆上，由 GC 管理。但很多对象其实只在一个方法内使用（方法返回后就没人引用了），或只在一个线程内使用（从不被其他线程看到），或加了 `synchronized` 但实际运行时没有竞争。对这些对象，堆分配 + GC 回收 + 锁操作全是浪费。

### 0.3 怎么解决？

**两层架构**：
- **字节码级**（`BCEscapeAnalyzer`）：快速保守分析，在方法首次被 CI 加载时执行，为 `ConnectionGraph` 提供跨方法参数逃逸信息
- **C2 IR 级**（`ConnectionGraph`）：精确分析，在 C2 编译的 `Optimize()` 阶段执行，基于 [Choi99] 论文的流无关分析

**五步流程**：填充连接图 → 传播引用（不动点迭代，通常 1-3 轮）→ 调整标量替换状态 → 优化 IR 图 → 类型分裂。

**两种优化**：
- **标量替换**：`NoEscape` 对象拆成独立字段，字段直接放寄存器/栈变量，对象消失
- **锁消除**：`NoEscape`/`ArgEscape` 对象的 Lock/Unlock/MemBar 全部删除

### 0.4 为什么这样设计？

- **为什么用流无关分析而不是流敏感分析？** 流敏感分析需要为每个程序点维护状态，复杂度高；流无关分析保守但快速，对大多数实际代码已经足够精确
- **为什么 HotSpot 11 没有栈上分配？** 标量替换比栈上分配更彻底——不仅避免了堆分配，字段还可能被优化到寄存器中，连栈空间都不需要；实现栈上分配还需要处理对象头、GC 扫描等复杂问题
- **为什么先消除锁再消除分配？** Lock/Unlock 节点引用了分配的对象，如果先消除分配，Lock 节点的输入就悬空了；先消除锁后，分配节点的使用者减少，标量替换更容易成功
- **为什么 EA 失败时自动关闭重试？** EA 是激进优化，可能导致节点数超限等编译失败；自动降级保证了编译的成功率，宁可不做 EA 也要生成可用的代码
