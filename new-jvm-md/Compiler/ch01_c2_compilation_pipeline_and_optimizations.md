...
C2 编译器的本质是一个**基于 Sea-of-Nodes IR 的激进优化编译器**：字节码 → Sea-of-Nodes 图（无基本块，数据节点自由漂浮）→ 多轮优化（IGVN/EA/循环优化/CCP）→ 指令选择（BURS 树匹配）→ 图着色寄存器分配 → 机器码。整个过程由 `Compile` 对象驱动，构造函数内完成全部编译。

### 0.2 为什么需要？

C1 编译器追求快速编译，生成的代码质量中等。服务端长期运行的热点方法需要**极致运行时性能**：

- **深度内联**：跨方法边界做优化（逃逸分析、常量传播）
- **循环优化**：展开/向量化/范围检查消除，大幅提升数组密集型代码性能
- **逃逸分析**：消除不必要的堆分配和锁操作
- **激进常量传播**：CCP 利用控制流信息做更精确的常量推断

### 0.3 怎么解决？

**Sea-of-Nodes IR**：数据节点"漂浮"在图中，不绑定到基本块，代码移动（code motion）成为图的自然属性，不需要单独的优化 Pass。

**三大优化钩子**：每个 Node 子类通过 `Ideal()`（图变换）、`Value()`（类型推断）、`Identity()`（恒等变换）三个虚方法参与优化，`PhaseIterGVN` 工作列表循环驱动这三个钩子反复运行直到不动点。

**多轮优化管道**：IGVN → 增量内联 → 逃逸分析 → 循环优化（4+ 轮）→ CCP → IGVN → 宏节点展开 → 指令选择 → 寄存器分配。

### 0.4 为什么这样设计？

- **为什么用 Sea-of-Nodes 而不是传统 SSA？** 传统 SSA 中每条指令有固定的基本块位置，代码移动需要单独的 Pass；Sea-of-Nodes 中纯计算节点自由漂浮，代码移动是图的自然属性，优化更自然
- **为什么用 BURS 树匹配而不是简单模式匹配？** BURS 是自底向上的最优树匹配，能找到代价最低的指令组合；简单模式匹配只能做局部最优
- **为什么用图着色而不是 Linear Scan？** C2 追求代码质量，图着色是最高质量的寄存器分配算法；C1 追求编译速度才用 Linear Scan
- **为什么循环优化要做 4+ 轮？** 每轮循环优化（展开/剥离/向量化）都会改变图结构，新的结构可能暴露新的优化机会，需要多轮迭代才能收敛到最优
...
public:
  const node_idx_t _idx;  // ★ 全局唯一的节点编号（编译期间单调递增）
...
**sizeof(Node)**：约 **48 字节**（6 个字段，GDB 验证：`p sizeof(Node)` = 48）

**创建位置**：`Parse::do_call()` / `GraphBuilder` 等前端代码中 `new XxxNode(...)` 创建，从 `node_arena`（`Amalloc_D`）分配，不走常规 C++ new/delete。

**关键字段生命周期**：
- `_in`/`_cnt`：构造时初始化；`set_req(i, n)` 修改输入边；`add_req(n)` 追加输入；`del_req(i)` 删除输入
- `_out`/`_outcnt`：`add_out(n)` 时追加（当 n 的某个输入指向 this 时）；`del_out(n)` 时删除；`outcnt() == 0` 表示死节点
- `_idx`：构造时从 `Compile::_unique` 原子递增分配；`destruct()` 时将 `_idx` 归还到 `_dead_node_list` 供复用
- `_in[0]`（控制输入）：数据节点为 NULL（漂浮）或指向控制节点；控制节点指向前驱控制节点；`PhaseCFG` 阶段将漂浮节点绑定到具体基本块
