...
`loadClass` 链路的本质是一个**两层委派模型**：外层是"先查缓存→委托 parent→自己加载"的骨架（传统双亲委派），内层是"先查模块映射表→直接定位目标 loader"的快捷路径（模块感知委派）。两者最终都通过 JNI 穿越到 C++ 层的 `SystemDictionary`。

### 0.2 为什么需要？

双亲委派解决的核心问题是**类的唯一性和安全性**：同一个类名在同一个 ClassLoader 下只能被加载一次，且核心类（`java.*`）只能由 Bootstrap Loader 加载，防止恶意代码冒充核心类。但传统双亲委派是单向的（只能向上委托），在 SPI 场景下（Bootstrap 加载的接口需要找到 App 加载的实现）和模块化场景下（跨层级委派）都力不从心。

### 0.3 怎么解决？

- **SPI 问题**：引入 Thread Context ClassLoader（TCCL），让 Bootstrap 加载的代码能通过线程获取 AppClassLoader 的引用，实现"向下委托"
- **模块化问题**：`BuiltinClassLoader` 引入 `packageToModule` 静态映射表，包名直接映射到目标 ClassLoader，跳过层级约束直接委托
- **并发问题**：`parallelLockMap` 将锁粒度从"整个 ClassLoader"细化到"per-class-name"，消除不同类加载之间的竞争

### 0.4 为什么这样设计？

- **为什么只重写 `findClass()` 而不是 `loadClass()`？** `loadClass()` 包含双亲委派骨架，重写它会破坏委派机制；`findClass()` 只负责"自己加载"这一步，重写它不影响委派逻辑
- **为什么 `parallelLockMap` 用 `ConcurrentHashMap` 而不是普通 `HashMap`？** 多线程同时加载不同类时，`putIfAbsent` 需要原子性；普通 `HashMap` 在并发写时会数据竞争
- **为什么 `resolveClass` 在 JDK 11 是 no-op？** JDK 11 采用 eager linking，类的链接在 `defineClass` 时已完成，`resolveClass` 保留只是为了 API 兼容性
...
| 结构名 | 源码位置 | 核心作用 |
|--------|----------|----------|
| `ClassLoader.parallelLockMap` | `ClassLoader.java:242` | per-class-name 锁映射表，null=非并行，非null=并行 |
| `BuiltinClassLoader.packageToModule` | `BuiltinClassLoader.java:140` | 包名→模块归属，三个 loader 共享，O(1) 跨级委派 |
| `LoadedModule` | `BuiltinClassLoader.java:内部类` | 模块归属三元组（loader + mref + codeSourceURL） |
| `URLClassPath` | `jdk/internal/loader/URLClassPath.java` | classpath 搜索器，AppClassLoader 独有 |
| `ClassFileStream` | `classfile/classFileStream.hpp` | 字节码流包装器，传递给 ClassFileParser |

### parallelLockMap 字段分析

**问题推导**：多线程同时加载不同类时，如果锁整个 ClassLoader（`synchronized(this)`），加载 `Foo` 的线程会阻塞加载 `Bar` 的线程——即使两者完全无关。需要将锁粒度细化到 per-class-name。
...
// ★ null  → 非并行 capable，loadClass 时锁 this
// ★ 非null → 并行 capable，loadClass 时锁 per-class-name 的 Object
```
...
|------|--------|----------|-----------|--------|
| ★ `parallelLockMap` | `registerAsParallelCapable()` | ClassLoader 子类的 static 块中 | `new ConcurrentHashMap<>()` | `getClassLoadingLock()` 判断锁粒度；VM C++ 层直接读取字段偏移 |

**VM 硬编码说明**：JVM C++ 层在解析常量池引用时，通过**固定字段偏移量**直接读取 `parallelLockMap`，不走 Java getter。这意味着 `parallelLockMap` 在 `ClassLoader` 类中的字段顺序不能随意调整。

### LoadedModule 字段分析

**问题推导**：`packageToModule` 表的 value 需要同时记录"哪个 loader 拥有这个模块"和"模块的资源在哪里"，以便后续加载类时既能委托给正确的 loader，又能读取模块内的字节码。

```java
// BuiltinClassLoader.java 内部类
private static class LoadedModule {
    private final BuiltinClassLoader loader;  // ★ 拥有此模块的 ClassLoader（跨级委派的目标）
    private final ModuleReference mref;       // 模块引用（含模块描述符 + 资源定位器 URI）
    private final URL codeSourceURL;          // 代码源 URL（ProtectionDomain 安全管理器用）
}
```

**sizeof 估算**：3 个引用字段 × 8 字节 = 24 字节（不含对象头 16 字节）= 约 40 字节

**创建位置**：`BuiltinClassLoader.loadModule(ModuleReference mref)` 中，每个模块创建一个 `LoadedModule`，然后将该模块的所有包名批量注册到 `packageToModule` 表。

**关键字段生命周期**：

| 字段 | 谁设置 | 何时设置 | 设置什么值 | 谁读取 |
|------|--------|----------|-----------|--------|
| ★ `loader` | `LoadedModule` 构造器 | `loadModule()` 调用时 | 调用 `loadModule()` 的那个 `BuiltinClassLoader` 实例 | `loadClassOrNull()` 中 `loadedModule.loader()` 判断是否需要跨级委派 |
| `mref` | `LoadedModule` 构造器 | 同上 | `ModuleBootstrap` 传入的 `ModuleReference` | `findClassInModuleOrNull()` 中通过 `mref` 获取 `ModuleReader` |
| `codeSourceURL` | `LoadedModule` 构造器 | 同上 | `mref.location().map(URI::toURL)` | `defineClass()` 中构造 `CodeSource` |
...
classDiagram
    ...
    class BuiltinClassLoader {
        ...
        +findClassInModuleOrNull(LoadedModule, String) Class
        +findClassOnClassPathOrNull(String) Class
    }
    ...
    class SystemDictionary {
        +find_instance_or_array_klass() Klass
        +resolve_from_stream() Klass
        +resolve_or_fail() Klass
    }
    ...
    BuiltinClassLoader --> URLClassPath : ucp (AppClassLoader only)
    ClassLoader ..> SystemDictionary : JNI (findLoadedClass/defineClass)
```

**关系说明**：
- `packageToModule` 是静态字段，三个 loader 共享，是跨级委派的核心
- `LoadedModule.loader` 指向拥有该模块的 ClassLoader，`loadClassOrNull()` 通过它决定是否需要跨级委派
- `URLClassPath` 只有 `AppClassLoader` 有值，`Boot/Platform` 为 null
- Java 层的 `findLoadedClass` 和 `defineClass` 都通过 JNI 穿越到 C++ 层的 `SystemDictionary`
...
| 结构 | 核心特征 |
|------|----------|
| `ClassLoader.parallelLockMap` | null=非并行（锁 this），非null=并行（per-class-name 锁）；VM 硬编码字段偏移直接读取 |
| `BuiltinClassLoader.packageToModule` | 静态全局共享，包名→模块归属，O(1) 查询，`initPhase2` 时一次性填充 |
| `LoadedModule` | 三元组（loader + mref + codeSourceURL），`loader` 字段是跨级委派的目标 |
| `URLClassPath` | 只有 AppClassLoader 有值，封装 classpath 搜索逻辑 |

### 算法层面

| 算法 | 核心设计决策 |
|------|-------------|
| `ClassLoader.loadClass()` | 三步骨架：查缓存→委托 parent→自己加载；`parallelLockMap` 决定锁粒度 |
| `BuiltinClassLoader.loadClassOrNull()` | 两条路径：模块路径（查表直接定位，可跨级）优先于 classpath 路径（传统双亲委派） |
| `getClassLoadingLock()` | `parallelLockMap != null` 时用 `putIfAbsent` 创建 per-name 锁，消除不同类加载间的竞争 |
| `findLoadedClass()` → `JVM_FindLoadedClass` | 纯内存查找（Dictionary 哈希表），不触发任何 I/O 或类解析 |
| `defineClass()` → `JVM_DefineClassWithSource` | Java 层做安全检查（`preDefineClass`），C++ 层做解析（`ClassFileParser`）和注册（`Dictionary::add_klass`） |
| TCCL（Thread Context ClassLoader） | 通过线程绑定的 ClassLoader 引用实现"向下委托"，解决 SPI 场景下 Bootstrap 无法访问 App classpath 的问题 |
