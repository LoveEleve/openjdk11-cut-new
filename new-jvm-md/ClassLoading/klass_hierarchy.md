...
Klass 层次结构的本质是 **JVM 对 Java 类型系统的 C++ 内存表示**：每个 Java 类/接口/数组类型在 Metaspace 中对应一个 `Klass` 对象，存储该类型的全部元数据（虚方法表、字段布局、继承关系、常量池等）。Java 对象（oop）通过对象头中的 klass 指针找到对应的 Klass，从而实现类型分发。

### 0.2 为什么需要？

**内存效率问题**：C++ 的做法是在每个对象头部放一个 vtable 指针（8 字节）。JVM 需要管理数十亿个 Java 对象，如果每个 Java 对象都嵌入 C++ vtable 指针，内存浪费巨大。同时，Java 对象还需要支持 `instanceof`/`checkcast`（类型检查）、`invokevirtual`/`invokeinterface`（虚方法分发）、GC 扫描（oop 字段定位）等操作，这些都需要类型元数据。

**核心矛盾**：Java 对象数量极多（数十亿），但 Java 类数量极少（数千）。把类型元数据放在每个对象里是极大的浪费。

### 0.3 怎么解决？

**oop/klass 二分法**：
- **oop 层**（Java 对象）：不含 C++ vtable 指针，只有 markWord + klass 指针 + 实例字段。数十亿个对象共享同一个 Klass
- **Klass 层**（类元数据）：含 C++ vtable（用于 C++ 多态分发），存储虚方法表/字段布局/继承关系等。每个 Java 类只有一个 Klass，存储在 Metaspace

**六种 Klass 子类**对应六种 Java 类型：`InstanceKlass`（普通类/接口）、`InstanceMirrorKlass`（java.lang.Class）、`InstanceRefKlass`（Reference 子类）、`InstanceClassLoaderKlass`（ClassLoader 子类）、`ObjArrayKlass`（对象数组）、`TypeArrayKlass`（基本类型数组）。

**三个关键设计**：
- `_layout_helper`：一个 int 编码对象大小/数组元素类型，对象分配时 O(1) 获取大小
- `_primary_supers[8]`：Cohen's Display 算法，`instanceof` O(1) 检查（继承深度 < 8）
- 嵌入式 vtable/itable/oop-map：紧跟 InstanceKlass header 之后，避免额外指针间接

### 0.4 为什么这样设计？

- **为什么 oop 不含 C++ vtable？** 节省内存。数十亿对象 × 8B = 数十 GB 浪费。所有"虚方法"转发到 Klass 上执行（`oop_` 前缀函数）
- **为什么 `_primary_supers` 限制 8 级？** 实际 Java 类继承深度中位数 3-4 级，超过 8 级极罕见。8 级覆盖 99%+ 场景，超出的用 `_secondary_supers` 线性搜索 + cache
- **为什么 vtable 嵌入在 InstanceKlass header 之后而不是独立分配？** 减少内存碎片和指针间接。`vtable_start_offset()` 是编译期常量，解释器/编译器直接用固定偏移访问，无需额外指针
- **为什么 ArrayKlass 的 vtable 也从 `InstanceKlass::header_size()` 偏移开始？** 统一所有 Klass 类型的 vtable 起始偏移，解释器和编译器只需一个固定常量，无需区分类型
- **为什么静态字段存在 java.lang.Class 实例（Java 堆）而不是 InstanceKlass（Metaspace）？** 静态字段可能是 oop 引用，GC 扫描 Java 堆时能自然发现；Metaspace 不在 Java 堆中，GC 不扫描
