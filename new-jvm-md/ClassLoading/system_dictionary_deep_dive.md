...
SystemDictionary 的本质是 **JVM 类加载的中央调度台**：它本身不存储已加载的类（类存储在各 ClassLoaderData 的 Dictionary 中），而是通过四张辅助表（PlaceholderTable/LoaderConstraintTable/ResolutionErrorTable/PD cache）协调类加载的全生命周期——从并发安全、循环依赖检测，到加载器约束校验和保护域缓存。

### 0.2 为什么需要？

类加载面临五个核心挑战，缺少任何一个都会导致 JVM 崩溃：

| 挑战 | 后果（如果没有 SystemDictionary） |
|------|----------------------------------|
| 多线程同时加载同一个类 | 内存中出现两个不同的 `InstanceKlass`，类型系统崩溃 |
| A 的父类是 B，B 的父类是 A | 无限递归加载，栈溢出 |
| 两个 ClassLoader 加载同名类 | 类型混淆，`ClassCastException` |
| 不同 ClassLoader 的方法签名引用同名类 | 链接时类型不一致，`LinkageError` |
| 已加载的类需要快速查找 | 每次引用都重新加载，性能灾难 |

### 0.3 怎么解决？

**协调者模式**：SystemDictionary 是 `AllStatic` 类（无实例），通过四张表分工协作：
- **Dictionary**（每 CLD 一个）：已加载类的哈希表，无锁快速查找（热路径 O(1)）
- **PlaceholderTable**：正在加载中的类的占位符，三种 action（LOAD_INSTANCE/LOAD_SUPER/DEFINE_CLASS）分别处理并发加载、循环依赖检测、并行 define 控制
- **LoaderConstraintTable**：跨加载器的类型一致性约束，确保不同 ClassLoader 对同名类的解析结果一致
- **PD cache**（DictionaryEntry 内嵌）：缓存已验证的 ProtectionDomain，避免重复 Java 层回调

**核心流程**：`resolve_instance_class_or_null` 的 8 阶段：无锁快速查找 → 锁策略选择 → 加锁二次检查 → 四种并发场景处理 → 实际加载 → 委派处理 → 清理 placeholder + notify_all → PD 验证。

### 0.4 为什么这样设计？

- **为什么 SystemDictionary 不存储类，而是通过每个 CLD 的 Dictionary 存储？** 类加载器隔离要求：不同 ClassLoader 加载的同名类是不同的类。每个 CLD 持有独立 Dictionary，ClassLoader 卸载时整批回收，无需遍历全局字典
- **为什么 PlaceholderTable 有三种 action？** 三种并发场景需要不同的协调语义：LOAD_INSTANCE 防止重复加载、LOAD_SUPER 检测循环依赖、DEFINE_CLASS 在 parallelCapable 加载器中序列化 define 操作
- **为什么无锁快速查找可以不加锁？** 三个保证：条目只在 safepoint 删除；读者不会在检查期间遇到 safepoint（NoSafepointVerifier）；新条目在对并发读者可见之前已完全构造好
- **为什么 `double_lock_wait` 如此复杂？** 传统 ClassLoader 持有对象锁，但等待时需要释放对象锁再等待 SystemDictionary_lock，否则死锁。必须记录递归次数、通知等待者、完全退出再重新进入，保证锁语义正确
