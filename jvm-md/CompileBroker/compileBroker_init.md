...
class CompileBroker: AllStatic {
 ...
 private:
  ...
  static volatile bool _should_block;          // 是否应该阻塞
  static volatile jint _should_compile_new_jobs; // 是否应该编译新任务
  ...
  // 编译器线程计数
  // =============================================
  static int _c1_count, _c2_count;             // C1/C2 线程数

  // =============================================
  // 编译器线程 Java 对象
  // =============================================
  static jobject *_compiler1_objects;          // C1 线程对象数组
  static jobject *_compiler2_objects;          // C2 线程对象数组

  // =============================================
  // 编译日志
  // =============================================
  static CompileLog **_compiler1_logs;         // C1 编译日志
  static CompileLog **_compiler2_logs;         // C2 编译日志

  // =============================================
  // 编译 ID 计数器
  // =============================================
  static volatile jint _compilation_id;        // 编译 ID
  static volatile jint _osr_compilation_id;    // OSR 编译 ID
...
class CompileTask : public CHeapObj<mtCompiler> {
 public:
  ...
  enum CompileReason {
      Reason_None,
      Reason_InvocationCount,  // 方法调用计数
      Reason_BackedgeCount,    // 循环回边计数
      Reason_Tiered,           // 分层编译
 ...
 private:
  static CompileTask* _task_free_list;  // 任务空闲列表（复用）
  
  Monitor*     _lock;                   // 任务锁
  uint         _compile_id;             // 编译 ID
  Method*      _method;                 // 要编译的方法
  jobject      _method_holder;          // 方法持有者
  int          _osr_bci;                // OSR 入口 BCI
  bool         _is_complete;            // 是否完成
  bool         _is_success;             // 是否成功
  bool         _is_blocking;            // 是否阻塞调用者
  int          _comp_level;             // 编译级别
  int          _num_inlined_bytecodes;  // 内联字节码数
  nmethodLocker* _code_handle;          // 结果 nmethod 持有者
  CompileTask* _next, *_prev;           // 链表指针
  bool         _is_free;                // 是否在空闲列表
  
  jlong        _time_queued;            // 入队时间
...
class CompileQueue : public CHeapObj<mtCompiler> {
 private:
  const char* _name;       // 队列名称
  CompileTask* _first;     // 队列头
  CompileTask* _last;      // 队列尾
  CompileTask* _first_stale; // 过期任务链表
  int _size;               // 队列大小
