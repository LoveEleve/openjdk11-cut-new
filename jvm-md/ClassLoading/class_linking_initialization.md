...
enum ClassState {
  ...
  initialization_error                // <clinit> 抛异常
};
```
...
```
┌─────────────────────────────────────────────────────┐
│ _indices (intx)  [ b2:8 | b1:8 | cp_index:16 ]     │  字节码 + 原始CP索引
├─────────────────────────────────────────────────────┤
│ _f1 (Metadata*)  Method*/Klass*                     │  解析后的元数据指针
├─────────────────────────────────────────────────────┤
│ _f2 (intx)       vtable_index / field_offset / ...  │  分派索引或偏移
├─────────────────────────────────────────────────────┤
│ _flags (intx)    [tos:4|F|M|A|I|f|v|vf|...|psize]  │  类型/标志位/参数
└─────────────────────────────────────────────────────┘
```
...
```
CallInfo {
  _resolved_klass   解析到的类（Methodref 中声明的类）
  _resolved_method  解析到的方法（声明中的方法）
  _selected_method  实际选择的方法（可能是子类覆盖后的版本）
  _call_kind         调用方式: direct / vtable_call / itable_call
  _call_index        vtable/itable 索引（direct 时为 Method::nonvirtual_vtable_index）
}
```

解释器根据 `CallInfo` 回填 CPCache entry：
- `direct` → `_f1 = Method*`
- `vtable_call` → `_f2 = vtable_index`（如果 vfinal 则 `_f2 = Method*`）
- `itable_call` → `_f1 = interface Klass*`, `_f2 = itable Method*`
