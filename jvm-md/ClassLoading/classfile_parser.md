...
```
┌─────────────────────────────────────────────────┐
│ ClassFileStream (extends ResourceObj)            │
├─────────────────────────────────────────────────┤
│ _buffer_start : const u1* const  ← 缓冲区起始   │
│ _buffer_end   : const u1* const  ← 缓冲区末尾+1 │
│ _current      : mutable const u1* ← 当前读取位置 │
│ _source       : const char* const ← 来源描述     │
│ _need_verify  : bool              ← 是否需要验证  │
└─────────────────────────────────────────────────┘
```
...
├────────────────────────────────────────────────────────┤
│ [0] access_flags      — 访问修饰 + HotSpot 扩展标志    │
│ [1] name_index        — 字段名在 CP 中的索引            │
│ [2] signature_index   — 类型描述符在 CP 中的索引        │
│ [3] initval_index     — ConstantValue 属性的 CP 索引    │
│ [4] low_packed_offset — 偏移低 16 位                    │
│ [5] high_packed_offset— 偏移高 16 位                    │
├────────────────────────────────────────────────────────┤
│ 解析阶段: [4][5] 暂存 FieldAllocationType               │
│ 布局阶段: [4][5] 被 set_offset(real_offset) 覆盖       │
└────────────────────────────────────────────────────────┘
```

如果字段有泛型签名（`JVM_ACC_FIELD_HAS_GENERIC_SIGNATURE`），其 `generic_signature_index` 存储在字段数组末尾的附加区域。

### 6.3 FieldAllocationType
