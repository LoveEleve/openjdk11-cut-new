...
HeapRegion *G1CollectedHeap::new_region(size_t word_size, bool is_old, bool do_expand) {
    ...
    if (res == NULL && do_expand && _expand_heap_after_alloc_failure) {
        ...
        log_debug(gc, ergo, heap)(
                "Attempt heap expansion (region allocation request failed). Allocation request: " SIZE_FORMAT "B",
...
HeapWord *G1CollectedHeap::humongous_obj_allocate(size_t word_size) {
    ...
    if (first == G1_NO_HRM_INDEX) {
        ...
        if (first != G1_NO_HRM_INDEX) {
            ...
            log_debug(gc, ergo, heap)(
                    "Attempt heap expansion (humongous allocation request failed). Allocation request: " SIZE_FORMAT "B",
                    word_size * HeapWordSize);
...
HeapWord *G1CollectedHeap::expand_and_allocate(size_t word_size) {
    ...
    log_debug(gc, ergo, heap)(
            "Attempt heap expansion (allocation request failed). Allocation request: " SIZE_FORMAT "B",
            word_size * HeapWordSize);
...
bool G1CollectedHeap::expand(size_t expand_bytes, WorkGang *pretouch_workers, double *expand_time_ms) {
    ...
    log_debug(gc, ergo, heap)(
            "Expand the heap. requested expansion amount: " SIZE_FORMAT "B expansion amount: " SIZE_FORMAT "B",
   ...
   */
    ...
    // forcus 计算需要扩展的region数(8GB下默认为2048个)
...
void G1CollectedHeap::shrink_helper(size_t shrink_bytes) {
    ...
    log_debug(gc, ergo, heap)(
            "Shrink the heap. requested shrinking amount: " SIZE_FORMAT "B aligned shrinking amount: " SIZE_FORMAT "B attempted shrinking amount: " SIZE_FORMAT "B",
...
G1CollectedHeap::G1CollectedHeap(G1CollectorPolicy *collector_policy) :
        ...
        _humongous_set("Master Humongous Set", true /* humongous */,
                       new HumongousRegionSetMtSafeChecker()), // note 巨型对象Region集合
        // note 巨型对象回收
...
jint G1CollectedHeap::initialize() {
    ...
    // forcus 预留虚拟内存 这里帮我纠正了我之前一直存在的一个误区，那就是我以为java堆(就是我们说的堆)是在 C 堆中再划分处理的一块内存区域
    // forcus 但是通过这里的源码，可以发现，java堆其实是位于 映射区的(而不是进程堆中的!)，这点是需要注意一下的
    /*
        void* mmap(
   ...
   */
    /*
        压缩指针模式:
         ...
         forcus 这里要介绍一下这个 ReservedSpace 对象
            ReservedSpace 是 JVM 中用于管理预留虚拟内存空间的核心类，它封装了操作系统级别的内存预留操作，并且能够保证地址空间的连续性
            {
                 ...
                 char*  _base;         // 预留空间的起始地址
                 size_t _size;         // 预留空间的大小
                 ...
            }
            两阶段内存分配策略:
             1. Reserve(预留)：mmap(),只占用虚拟地址空间,不分配物理内存，不消耗RSS
             2. Commit(提交)：mmap(),修改页表权限，触发page fault时分配物理页

            ReservedSpace 与 G1RegionToSpaceMapper 之间的关系是什么呢？
                这两者通常是协作关系：
                    1. 先预留虚拟地址空间
                        ReservedSpace heap_rs = Universe::reserve_heap(xxx, alignment);
                    2. 创建映射器管理这段空间
                        G1RegionToSpaceMapper* heap_storage =
                                G1RegionToSpaceMapper::create_mapper(
                                    heap_rs,                    // ← 传入 ReservedSpace
                                    heap_rs.size(),
                                    page_size,
                                    HeapRegion::GrainBytes,
                                    1,
                        ...
                        然后在 映射器内部保存 ReservedSpace
                        class G1RegionToSpaceMapper {
                        private:
                            ...
                            ReservedSpace _storage;
                            ...
                        }
   */
    /*
        在上面讲解了 ReservedSpace 和 G1RegionToSpaceMapper 之间的关系，但是不要忘记了之前在初始化G1 Heap的时候,还涉及到了一个类型对象
            - MemRegion: 对这个类型对象的描述是 - 表示一段连续的内存区域,那么和这里的2个类型对象之间有关系吗?
                - 这里对这个类型对象的描述再补充一下：轻量级内存区域描述符
                {
                    HeapWord* _start;      // 起始地址
                    size_t    _word_size;  // 大小（以 HeapWord 为单位,8字节）
                }
                - 重点在于其仅描述一段内存范围，不管理内存生命周期，JVM 各处都在使用，不限于 G1
                    - MemRegion heap_region(heap_start, heap_size_in_words ：描述堆的预留区域
                    - MemRegion obj_region(obj_addr, obj_size_in_words) ： 描述单个对象占用的内存
                    - void process_memory(MemRegion mr) ： 作为函数参数传递内存范围
                ====
            - MemRegion - 内存范围描述者，轻量级描述内存范围，不管理生命周期
            - ReservedSpace - 虚拟内存管理者，管理虚拟地址空间的生命周期
            - G1RegionToSpaceMapper - 物理内存管理者，管理虚拟地址到物理内存的映射
   */
   ...
   *
   */
    G1CardTable *ct = new G1CardTable(
            reserved_region()); // 这里传入了 reserved_region() - 就是上面说的 _reserved对象(内部包含了堆的起始地址和大小(堆的范围))
   ...
   */
    /*
        forcus 这里还需要额外注意一点: 创建下面的6个映射器使用了两种不同的方式,这两种有什么区别呢?
            1. create_mapper():直接创建，使用已有的 ReservedSpace，比如下面的 heap_storage 就是用来管理堆内存本身的
            2. create_aux_memory_mapper(): 辅助结构创建(BOT,卡表，位图)
                内部会创建新的 ReservedSpace(调用new ReservedSpace()来创建独立的虚拟地址空间)
                独立的内存区域，与堆内存分离，有自己的地址范围
                但是还是会继续调用 G1RegionToSpaceMapper::create_mapper()来创建映射器的
   */
    // note 图解-4
    G1RegionToSpaceMapper *heap_storage =
   ...
   */
    ...
    /*
   * note 这里还是要重新研究一下,这里创建的两个映射器的作用：prev_bitmap_storage && next_bitmap_storage
   *  问题场景: 并发标记需要记录哪些对象是存活的？
   *    在 G1 的并发标记阶段，GC 线程需要遍历整个对象图，找出所有存活的对象
   *    核心问题：如何记录一个对象是否已经被标记过？
   *    {堆中有大量对象，GC 需要：从 GC Roots 开始遍历， 对每个访问到的对象，标记为"存活"  ，后续阶段根据标记结果，回收未标记的对象 }
   *    问题：如何高效地存储和查询每个对象的"是否存活"状态？
   *    方案 A：在对象头中加一个标记位
   *        缺点：并发修改对象头需要复杂的同步机制 / 无法保留多轮标记的结果
   *    方案B : 使用独立的标记位图(G1选择了这种)
   *        1 个 bit 对应一小块堆内存，bit = 1 表示这块内存的起始位置有一个存活对象，与对象内存分离，支持原子操作，可以有多份
   *    新的问题又出现了: 一个位图不够用
   *        场景：并发标记与混合回收同时需要使用位图
   *        Mixed GC 需要【读取】上一轮完成的、稳定的标记结果,并发标记需要【写入】当前正在进行的标记结果 ,如果只有一个位图，读和写会相互干扰！
   *    解决方案：G1 引入了双缓冲机制：使用两个位图！
   *        1. prev_mark_bitmap(上一轮的结果,只读)：Mixed GC 使用，用来判断对象是否存活
   *        2. next_mark_bitmap(当前正在进行的,可写)：并发标记线程 ， 标记新发现的对象
   *        标记周期完成时，只需交换两个指针：O(1) 操作！只交换指针，不复制数据
   *    在G1中，每64字节堆内存对应一个bit,位图大小 = 8GB / 64B = 128MB(双缓冲的话则需要256M)
   */
    /*
     * 在阅读jvm源码的时候，经常会碰到初始化某些变量,而这些变量又与G1息息相关，所以在这里先简单的了解一下g1 gc相关概念(这里先不深入细节，而是了解大概都做了什么)
     *    1. G1的核心思想: 逻辑上分代,但是内存不分，传统gc是将堆内存划分为年轻代和老年代，每次回收都要一次性处理整个代，粒度太粗了
     *                    而g1则将堆内存划分为一个个的Region(也即内存region化)，每个region都可以充当不同的代(在g1中有 -- Eden(新对象分配))
     *                    Survivor(存活对象),Old(长期存活对象)，Humongous(大对象，大小超过region一半)
     *    2. G1的两个核心目标：
     *        a. 暂停时间目标：用户设定期望暂停时间 (-XX:MaxGCPauseMillis=200)
     *        b. 增量回收：每次只回收收益最高的部分Region，而非全部
     *    3. G1 GC类型
     *        - Young GC(STW):
     *            1. Normal: 普通 Young GC，只回收年轻代 Region
     *            2. InitialMark: Initial Mark 暂停，搭便车在 Young GC 上执行
     *            3. DuringMarkOrRebuild: 并发标记进行中的 Young GC
     *        - Mixed GC (STW)
     *            1. 回收年轻代 + 部分老年代 Region（根据回收收益选择）
     *        - Full GC (STW) - 最后的保底手段
     *            1. 回收整个堆，压缩内存，非常慢！
     *        - Concurrent Marking (并发)
     *            1. 与应用程序并发执行，标记老年代中的存活对象
     *    4. G1 的两个阶段 (G1 的工作可以分为两个交替进行的阶段)
     *            1. Young-Only Phase(只有年轻代阶段)
     *                    只进行 Young GC，回收年轻代 Region
     *                    随着应用运行，老年代逐渐被填满
     *                    当老年代占用达到阈值（IHOP）时，触发并发标记
     *                    包含的 GC 类型：
     *                        Normal Young GC
     *                        Initial Mark（并发标记的起点）
     *                        DuringMarkOrRebuild（并发标记进行中）
     *                            结论验证：并发标记包含以下阶段：
                                      Initial Mark (STW) - 搭载在 Young GC 上
                                      Scan Root Regions (并发) - 扫描 Survivor 区
                                      Concurrent Mark (并发) - 遍历对象图，标记存活对象
                                      Remark (STW) - 完成标记，处理 SATB 队列
                                      Rebuild Remembered Sets (并发) - 重建记忆集
                                      Cleanup (STW) - 清理，计算回收收益，准备 Mixed GC ✓
     *               ====> 并发标记完成后,进入到第二阶段
     *            2. Space Reclamation Phase(空间回收阶段)
     *                    进行 Mixed GC，回收年轻代 + 部分老年代 Region
     *                    每次选择"垃圾最多"的老年代 Region 加入回收集合
     *                    持续进行直到回收收益低于阈值
     *                    然后回到 Young-Only Phase
     *                    包含的 GC 类型：
     *                        Mixed GC
     *               ====> 又回到 Young-Only-Phase阶段
     *
     *             ===== 继续补充
     *             G1 在 VM层面的3种GC操作：
     *                1. VM_G1CollectForAllocation：分配失败时触发，核心入口,包含了Young GC 和 Mixed GC
     *                2. VM_G1CollectFull：严重内存不足，Full GC,最后的保底手段
     *                3. VM_CGC_Operation: 并发标记期间，执行Remark和Cleanup暂停
     *            四种GC类型: 都需要STW
     *                1. Normal：普通 Young GC
     *                2. InitialMark：初始标记（搭载在 Young GC 上）
     *                3. DuringMarkOrRebuild：并发标记进行中的 Young GC
     *                4. Mixed： 混合 GC
     *            两个阶段：
     *                1. Young-Only:执行的GC有：Normal / InitialMark / DuringMarkOrRebuild 类型的, 只回收年轻代
     *                2. Space Reclamation：执行的GC有Mixed，回收年轻代 + 部分老年代
     *            两个阶段如何切换呢？当老年代占用 > IHOP 阈值时，并发标记开始(Young GC + Initial Mark) 如下面的一个注释(但是不一定准确)
     *              触发的详细条件：
     *                  1. 老年代占用 > IHOP 阈值（Initiating Heap Occupancy Percent，默认 45%）
     *                  2. 当前处于 Young-Only Phase
     *                  3. 没有正在进行的并发标记周期
     */
    ...
    /*
        ...
        │   │  ════════════ Concurrent Marking ════════════                              │
        │   │  │                                                                         │
        │   │  │  [Young GC]  [Young GC]  ...  （并发标记同时进行）                       │
        │   │  │     ↓           ↓                                                       │
        │   │  │   DuringMark  DuringMark                                                │
        │   │  │                                                                         │
        │   │  │  Remark (STW) → Cleanup (STW)                                           │
        │   │  │     ↓                                                                   │
        │   │  ════════════════════════════════════════════                              │
        │   │                                                                             │
   ...
   */
    _hrm.initialize(heap_storage, prev_bitmap_storage, next_bitmap_storage, bot_storage, cardtable_storage,
                    card_counts_storage);
   ...
   */
...

    {
        ...
        /*
     * 为什么需要这个?
     *  - 问题背景: 在垃圾收集过程中，需要频繁判断某个对象/区域是否在当前的收集集合(Collection Set)
     *  这个操作是非常频繁的,发生在很多地方:
     *      1. 写屏障：每次引用赋值 obj.field = another_obj 时，需要判断
     *          问题：如果 Region B 在 CSet 中，obj_a → obj_b 这条引用必须被记录到 RSet！，否则 GC 时会漏掉 obj_b，导致 obj_b 被错误回收！
     *          所以：每次执行 obj_a.field = obj_b 时，都需要判断：， "obj_b 所在的 Region 是否在 CSet 中？"
     *      2. 对象复制：标记阶段遍历对象图时，需要判断每个引用指向的对象
     *      3. 根扫描：扫描 GC Roots 时，需要判断每个根引用
     *
     *      如果没有这个属性，那么正常情况下，应该如何判断某个对象/region 是否在当前的CSet中呢？
     *          CSet中可能存在多个Region，每判断一个对象/Region是否在CSet中，都要遍历整个CSet(又是遍历？是绝对不可能遍历的啊)，而这个操作在一次GC中可能指向数百万次
     *          每一次都要进行O(n)的遍历，性能急剧下降啊
     *      老办法,用内存来换时间,给一个对象地址 addr ，我希望O(1)判断这个对象是否CSet中
     *      而这个牺牲的内存就是 _in_cset_fast_test -> 8GB,2048个region，所以长度就是2048，每个元素对应着region是否在CSet中
     *      有4个值：
     *          0: NotInCSet,不在CSet中
     *          1：Young,年轻代区域，并且在CSet中
     *          2: Old ，老年代区域，并且在CSet中
     *          -1: Humongous,巨型对象区域,特殊处理
     *      在这里只记录了一个Region是否在CSet中，那对于对象呢？这里要执行两步操作:
     *          1. 对于给定的addr,找到其所在的region，同样这里也是分配2048的数组长度，而对于给定的addr直接计算索引 >>22是不正确的
     *              需要 size_t index = (addr - heap_start) >> 22
     *              然后访问对应的 _in_cset_fast_test[index] = 1 (代表该对象所在的是一个Young Region，并且在CSet中)
     *              同样，这里不要做减法，也是一开始就做虚拟数组地址计算：_base(这里的_base是 _in_cset_fast_test 内部的数组) - heap_start >> 22
     *          直接O(1)判断出一个对象是否在CSet中
     */
        _in_cset_fast_test.initialize(start, end, granularity);
        // forcus 初始化巨型对象回收候选数组，用于标记可回收的巨型区域
        // note 巨型对象跨越多个区域，需要特殊的回收策略和跟踪机制
        /*
     * 首先要介绍一下:什么是 Humongous 对象呢？为什么需要专门跟踪呢？
     * 在G1 GC中，一个对象的大小 如果 >= Region大小 的50%,就被称为 Humongous（巨型）对象  (普通对象只会在一个region中，而 Humongous对象 可能会跨越多个region)
     *  问题场景：虽然对象很大，但是不可能不回收吧，那么GC时就需要判断哪些 Humongous对象 能够被回收
     *  传统 GC 对 Humongous 对象的处理方式存在 3 个问题
     *      1. 回收时机太晚
     *          传统上，Humongous 对象被当作老年代对象
     *          需要等到 Mixed GC 或 Full GC 才能回收
     *          但大部分 Humongous 对象生命周期很短（比如临时创建的大数组
     *      2. 遍历代价太高
     *          如果 Humongous 对象是一个大数组（如 byte[10MB]）
     *          标记阶段需要扫描它引用的所有对象
     *          对于纯数据数组，这完全是浪费
     *  解决方案: Eager Reclaim（快速回收)
     *      如果一个 Humongous 对象没有被任何其他对象引用
     *      并且它是纯数据类型数组（typeArray，如 byte[]、int[]）
     *      可以在 Young GC 期间直接回收它，不需要等到 Mixed GC
     *
     *      3.如何高效判断哪些 Humongous Region 可以快速回收？这就是 humongous_reclaim_candidates 的作用
     *      场景：在 Young GC 开始时，需要快速筛选出哪些 Humongous Region 是回收候选。
     *      如果没有这个数据结构：
     *          1. 需要遍历所有 Region，逐个检查是否是 Humongous
     *          2. 需要逐个检查每个 Humongous 对象的类型和引用情况
     *          3. 这个过程很慢，影响 GC 暂停时间
     *   有了: humongous_reclaim_candidates
     *          1. 每个 Region 只需 1 字节（bool）标记是否是回收候选（高效的判断哪些Region是回收候选）
     *          2. 使用偏置基地址优化，O(1) 时间判断
     *          3. 配合 _has_humongous_reclaim_candidates 标志，如果没有候选可以跳过整个流程
     *   什么样的 Humongous 对象 可以成为回收候选呢？
     *      1. 必须是 typeArray（纯数据类型数组）
     *          byte[]、char[]、int[] 等纯数据数组内部没有对象引用
                回收时不需要清理记忆集中指向其他 Region 的引用
                如果是 Object[] 数组，内部有引用，清理记忆集代价太高
     *      2. 记忆集(RSet足够小)
     *          为什么要检查记忆集？
                    如果有很多其他 Region 引用这个 Humongous 对象，说明它可能还活着
                    如果记忆集为空或很小，说明几乎没有引用，可以安全回收 ? AI给出的这段话,明显是错误的
                    AI的二次分析:
                        真正的两阶段判断逻辑:
                            1. 注册候选（GC 开始前），在 humongous_region_is_candidate() 中判断是否可以成为候选
                                此时确实为上面两个条件：必须为纯数据类型 & 记忆集足够小
                            2. 实际回收(gc过程中)，在真正回收时,是有额外的严格检查的,也即记忆集必须为空(避免将存活对象给回收掉了)
                            第一阶段只是初步筛选,排除哪些明显有大量被引用的 humongous 对象，目的是为了减少后续第二阶段(gc)检查的工作量，提升gc性能
     */
   ...
   */
    /*
     * note 并发标记带来的挑战：
     *  1. 对象引用发生变化(漏标问题)：使用SATB(Snapshot At The Beginning)队列
     *  2. 如何记录每个对象的标记状态(堆中有数百万个对象，需要快速查询/设置某个对象是否被标记/还有支持并发读写)：使用标记位图(Mark Bitmap)
     *  3. 当前周期和上一周期的标记信息冲突：解决方案:双缓冲位图(prev_bitmap & next_bitmap)
     *  4. 如何组织待处理的对象:标记采用的是三色标记发,对于灰色对象来说是还需要继续处理的,使用 全局标记栈 + 任务队列来存储灰色对象引用
     *  5. 如何利用多核性能：任务队列集合 + 工作窃取
     *  而这个 G1ConcurrentMark 就是用来初始化这些组件的
     */
   ...
   *   3. 提交所有辅助数据结构的内存
   */
    if (!expand(init_byte_size, _workers)) { // 这里的 _workers是 GC Thread(13个)
...
void G1CollectedHeap::iterate_dirty_card_closure(CardTableEntryClosure *cl, uint worker_i) {
    ...
    g1_policy()->phase_times()->record_thread_work_item(G1GCPhaseTimes::UpdateRS, worker_i, n_completed_buffers,
                                                        G1GCPhaseTimes::UpdateRSProcessedBuffers);
...
size_t G1CollectedHeap::recalculate_used() const {
    ...
    g1_policy()->phase_times()->record_evac_fail_recalc_used_time(
            (os::elapsedTime() - recalculate_used_start) * 1000.0);
    return blk.result();
}
    ...
    // Inner scope for scope based logging, timers, and stats collection
    {
        ...
        { // Call to jvmpi::post_class_unload_events must occur outside of active GC
...

            {
                ...
                size_t total_cards_scanned = g1_policy()->phase_times()->sum_thread_work_items(G1GCPhaseTimes::ScanRS,
                                                                                               G1GCPhaseTimes::ScanRSScannedCards);
...
void G1CollectedHeap::restore_after_evac_failure() {
    ...
    g1_policy()->phase_times()->record_evac_fail_remove_self_forwards(
            (os::elapsedTime() - remove_self_forwards_start) * 1000.0);
}
...
public:
    G1ParTask(G1CollectedHeap *g1h, G1ParScanThreadStateSet *per_thread_states, RefToScanQueueSet *task_queues,
              G1RootProcessor *root_processor, uint n_workers)
...
public:
    G1StringAndSymbolCleaningTask(BoolObjectClosure *is_alive, bool process_strings, bool process_symbols,
...
Monitor *G1CodeCacheUnloadingTask::_lock = new Monitor(Mutex::leaf, "Code Cache Unload lock", false,
...
private:
    ...
    public:
        ...
        virtual bool do_heap_region(HeapRegion *r) {
            ...
            if (r->is_young()) {
                assert(r->young_index_in_cset() != -1 &&
                       (uint) r->young_index_in_cset() < g1h->collection_set()->young_region_length(),
        ...
        void complete_work() {
            ...
            g1h->alloc_buffer_stats(InCSetState::Old)->add_failure_used_and_waste(_failure_used_words,
    ...
    private:
        ...
        WorkItem *_work_items;
    public:
        G1PrepareFreeCollectionSetClosure(WorkItem *work_items)
                : HeapRegionClosure(), _cur_idx(0), _work_items(work_items) {}
...
public:
    G1FreeCollectionSetTask(G1CollectionSet *collection_set, EvacuationInfo *evacuation_info,
...
void G1CollectedHeap::free_collection_set(G1CollectionSet *collection_set, EvacuationInfo &evacuation_info,
                                          const size_t *surviving_young_words) {
    _eden.clear();
...
public:
    ...
    G1FreeHumongousRegionClosure(FreeRegionList *free_region_list) :
            _free_region_list(free_region_list), _humongous_objects_reclaimed(0), _humongous_regions_reclaimed(0),
            _freed_bytes(0) {
    }
    ...
    virtual bool do_heap_region(HeapRegion *r) {
        ...
        if (!g1h->is_humongous_reclaim_candidate(region_idx) ||
            ...
            log_debug(gc, humongous)(
                    "Live humongous region %u object size " SIZE_FORMAT " start " PTR_FORMAT "  with remset " SIZE_FORMAT " code roots " SIZE_FORMAT " is marked %d reclaim candidate %d type array %d",
                    region_idx,
        ...
        log_debug(gc, humongous)(
                "Dead humongous region %u object size " SIZE_FORMAT " start " PTR_FORMAT " with remset " SIZE_FORMAT " code roots " SIZE_FORMAT " is marked %d reclaim candidate %d type array %d",
                region_idx,
                (size_t) obj->size() * HeapWordSize,
                p2i(r->bottom()),
                r->rem_set()->occupied(),
...
HeapRegion *G1CollectedHeap::alloc_highest_free_region() {
    ...
    if (index != G1_NO_HRM_INDEX) {
        if (expanded) {
            log_debug(gc, ergo, heap)(
                    "Attempt heap expansion (requested address range outside heap bounds). region size: " SIZE_FORMAT "B",
