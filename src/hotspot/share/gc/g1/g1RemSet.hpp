...
private:
    G1RemSetScanState *_scan_state; // forcus 核心数据结构 - 管理 GC 期间的记忆集扫描状态  / 跟踪每个 Region 的扫描进度 /  记录需要清理卡表的"脏区域"
    ...
    G1CollectedHeap *_g1h; // g1堆的引用
    size_t _num_conc_refined_cards; // Number of cards refined concurrently to the mutator.  并发精炼的卡片数量计数器(用于日志统计)

    G1CardTable *_ct; // 卡表引用(用于查找脏卡,计算卡地址)
    G1Policy *_g1p; // gc策略引用(用于记录gc阶段时间统计)
    G1HotCardCache *_hot_card_cache; // 热卡缓存引用,用于并发精炼时识别和缓存热卡
