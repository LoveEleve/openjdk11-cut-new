...
void ThreadLocalAllocBuffer::initialize() {
    // forcus 把指针都设为 NULL（还没有真正的内存）
    /**
        {
            _start / _top / _pf_top / _end / _allocation_end = null
            此时tlab是空的，等线程第一次分配对象的时候，才会去Eden区申请真正的内存
        }
     */
  initialize(NULL,                    // start
             NULL,                    // top
             NULL);                   // end
    // forcus 计算并设置期望大小 2MB
  set_desired_size(initial_desired_size());

  // Following check is needed because at startup the main
  // thread is initialized before the heap is.  The initialization for
  // this thread is redone in startup_initialization below.
  // forcus 计算并采样分配比例 24.5% 预计这个线程会使用约 24.5% 的 Eden 空间
  /*
        用于后续动态调整 TLAB 大小
        如果一个线程分配很多，就给它更大的 TLAB
        如果分配很少，就给它更小的 TLAB
   */
  if (Universe::heap() != NULL) {
    ...
    /*
        场景：TLAB 快用完了，要分配一个 40KB 的对象
            情况1：剩余空间 = 50KB（> 32KB 浪费限制）
              → 不换 TLAB，直接在 Eden 慢路径分配这个对象
              → 因为丢掉 50KB 太浪费了

            情况2：剩余空间 = 20KB（< 32KB 浪费限制）
              → 换一个新的 TLAB
              → 因为剩余空间不多，浪费可接受
     */
  set_refill_waste_limit(initial_refill_waste_limit());
...
void ThreadLocalAllocBuffer::startup_initialization() {
  ...
  // 1/2 full at a GC
  _target_refills = 100 / (2 * TLABWasteTargetPercent); // 50 (TLABWasteTargetPercent默认值为1)
  // We need to set initial target refills to 2 to avoid a GC which causes VM
  // abort during VM initialization.
  _target_refills = MAX2(_target_refills, 2U); // 50

  /*
        forcus 创建统计信息收集器(记录所有的TLAB的使用情况)
            - 有多少线程在分配对象？
            - 总共换了多少次 TLAB？
            - 总共分配了多少内存？
            - 浪费了多少空间？
            - GC 时剩余的空间
            - 换 TLAB 时剩余的空间
            这些统计数据可以通过 jstat 工具查看，帮助调优
   */
  _global_stats = new GlobalTLABStats();

  /**
        note 为 C2 编译器预留"安全空间"
        背景知识：C2 是 JVM 的高性能编译器，它会生成预取指令
        什么是预取？
            普通分配（不预取）:
                1. 分配对象位置
                ...
                3. 写入数据
            预取分配（提前告诉内存）:
                1. 预取指令: "嘿内存，我待会要用后面的空间，先准备好"
                2. 分配对象位置
                3. 写入数据  ← 内存已经准备好了，不用等！
            问题：预取指令会访问 TLAB 后面的内存
            TLAB 内存布局:
            ┌─────────────────────────────────┬────────────┐
            │         可分配区域              │  保留区    │
            │                                 │            │
            ...
            如果没有保留区，预取指令可能访问 TLAB 外面，导致程序崩溃！
            作用：计算需要预留多少空间，防止预取指令越界崩溃。
  ...
  // forcus 重新初始化主线程的 TLAB(因为此时G1 堆已经初始化完毕了)
