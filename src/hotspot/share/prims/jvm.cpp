...
JVM_ENTRY(jclass, JVM_DefineClassWithSource(
        JNIEnv * env, const char *name, jobject loader, const jbyte *buf, jsize len, jobject pd, const char *source))
...
JVM_ENTRY(jobject, JVM_GetStackAccessControlContext(JNIEnv * env, jclass cls))
    ...
    if (local_array->is_empty()) {
        ...
        oop result = java_security_AccessControlContext::create(objArrayHandle(), is_privileged,
                                                                privileged_context, CHECK_NULL);
        return JNIHandles::make_local(env, result);
    }
...
JVM_ENTRY(jobjectArray, JVM_GetMethodParameters(JNIEnv * env, jobject method))
    {
        ...
        if (num_params < 0) {
            ...
            objArrayOop result_oop = oopFactory::new_objArray(SystemDictionary::reflect_Parameter_klass(),
                                                              num_params, CHECK_NULL);
...
// zero for arrays.
JVM_QUICK_ENTRY(void, JVM_GetMethodIxExceptionIndexes(JNIEnv * env, jclass cls, jint method_index, unsigned short *
                                                                                                   exceptions))
...
JVM_QUICK_ENTRY(void, JVM_GetMethodIxExceptionTableEntry(JNIEnv * env, jclass cls, jint method_index, jint entry_index,
...
static void thread_entry(JavaThread *thread, TRAPS) { // 参数为jvm中的JavaThread对象
    HandleMark hm(THREAD);
    Handle obj(THREAD, thread->threadObj()); // obj()代表的是 java.lang.Thread对象
    JavaValue result(T_VOID);
    // focus
    // 调用java中的run()方法
    JavaCalls::call_virtual(&result, // 存储返回值
                            obj, // java.lang.Thread对象
                            SystemDictionary::Thread_klass(), // java.lang.Thread 类的 Klass 对象
...
JVM_ENTRY(void, JVM_StartThread(JNIEnv * env, jobject jthread))
    ...
    // in Thread::start.
    {
        ...
        // we operate.
        // JVM维护了全局的线程列表,创建新线程时需要修改这个列表,在这里需要保证线程安全
        ...
        // 检查线程是否已经启动过了,如果启动过了,设置 throw_illegal_thread_state = true,在后面会抛出异常
        if (java_lang_Thread::thread(JNIHandles::resolve_non_null(jthread)) != NULL) {
            ...
            // focus 创建 JavaThread对象
            // 在这里传入了 thread_entry(在这里会回调run()方法)
