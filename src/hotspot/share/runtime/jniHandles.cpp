 ...
 * forcus jni handle
 *  1. 简介:什么是jni handle呢？
 *      当 native 代码（C/C++）需要持有 Java 对象引用时，不能直接保存对象指针（因为 GC 会移动对象）。必须通过 JNI Handle 间接引用
         Native 代码                    JNI Handle Block                Java 堆
                                      ┌─────────────────┐
        jobject obj ──────────────────┤ handle[0] ──────┼──────→ Java 对象 A
                                      ├─────────────────┤
        jobject obj2 ─────────────────┤ handle[1] ──────┼──────→ Java 对象 B
                                      ├─────────────────┤
                                      │ handle[2]       │
                                      │ ...             │
                                      │ handle[31]      │  (每块 32 个槽位)
...
JNIHandleBlock *JNIHandleBlock::allocate_block(Thread *thread) {
    ...
    // forcus 如果thread对象不为null,则优先从线程本地空闲列表获取（无锁，快速）
    if (thread != NULL && thread->free_handle_block() != NULL) {
        block = thread->free_handle_block();
        thread->set_free_handle_block(block->_next);
    } else { // forcus thread对象为null时,一般第一次会进入到else分支,也即从全局free list(全局空闲链表)中获取 - 一旦涉及到全局/‘global’，那么就需要加锁
        ...
        MutexLockerEx ml(JNIHandleBlockFreeList_lock,
                         Mutex::_no_safepoint_check_flag); // 加锁,保证线程安全
        if (_block_free_list == NULL) {
            // Allocate new block
            block = new JNIHandleBlock(); // forcus 一般第一次都为null,创建一个JNIHandleBlock对象(内部是一个32个槽位的数组)
            _blocks_allocated++; // block计数
            block->zap(); // 初始化block对象
#ifndef PRODUCT
            // Link new block to list of all allocated blocks
            block->_block_list_link = _block_list; // 链接到JNIHandleBlock::_block_list链表上去
            _block_list = block;
#endif
        } else {
